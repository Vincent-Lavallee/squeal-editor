...
| menu | The titlebar's menus are inline in `Menu`; every context menu is `<ContextMenu>` above. Floating is what earns the `BORDER_STRONG` outline rather than a shadow. Menu items track their own hover state. Three of them, left to right: **File**, **Preferences**, **About**. |

...
| run group | Inline in `EditorPane`. An accent-filled flex box with the rounded ends, holding a primary `<Button>` and a `<Select variant="attached" caretOnly>` that each draw no box of their own. The divider between them is 1px of `color-mix(in srgb, currentColor 35%, transparent)` — the "structure from borders" rule applied *inside* a filled control, where a `--border` grey would read as a gap between two buttons rather than as one shape. **The attached half is the caret and nothing else**: the database's name is the toolbar's own label at the far left, because a second piece of high-contrast content inside the loudest control on screen makes it shout. The `attached` variant carries `color: inherit` — that is what makes the arrow exactly the same white as the *Run* label beside it, rather than `--text` sitting on an accent fill. See `docs/decisions.md`. |
| toolbar database label | Inline in `EditorPane`, first thing in the bar, before the spacer. The name of the database the tab runs against, at `TEXT_LABEL` (11px) in `--text-muted` — the one place that size is used **without** the uppercase and letter-spacing the token is otherwise for, because it is a name the server gave us and casing anything the server said is what the value rules forbid. A label, not a control: the caret on Run, the sidebar picker and `Ctrl+D` are the three ways to change it. |

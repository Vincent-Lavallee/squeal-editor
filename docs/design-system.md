...
in the system. It is not a second surface: it is light added on top of the one
background, the `--veil-sheen` family, and what it means is "this area pans and
zooms" — which is a real fact about the one view that has it rather than
decoration. It is grayscale, so rule 2 holds. It sits a step above `--hover`
because a 4% dot at 24px spacing is invisible, and a grid nobody can see is
worse than no grid.

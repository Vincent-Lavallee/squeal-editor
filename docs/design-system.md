...
| Where | Contains |
|---|---|
| `tokens.ts` | names every colour, size, radius a component may spend. A colour export is a `var(--token)` reference; a size is a plain number. |
| `residual.css` | holds the actual colour values, one block per theme (`:root` for dark, `:root[data-theme='light']` for light) — the one place a colour is *written*. Also CSS custom properties for size/shape (Monaco + the window frame read the colours at runtime), reset, scrollbars, `::placeholder`, `:focus-visible`, `@keyframes`, `:has()`, and grid compound selectors. |
| `components/` | reusable primitives that own their styles: `<Button>`, `<Input>`, `<Select>`, `<Checkbox>`, `<Badge>`, `<Modal>`, `<Drawer>`, `<ContextMenu>`, `<Note>`, `<Callout>`, `<Field>`, `<ResizeHandle>`, `<SrOnly>`, `<Mono>`, `<Tooltip>` |
| Inline in components | layout and custom styling — every component owns its own layout |

**Never hardcode a colour or size in a component.** If a token is missing, add
its name to `tokens.ts` and its value to *both* blocks in `residual.css`.
...
The window frame colour still travels across the bridge: `useFrameColour.ts`
reads `--bg` back off the DOM and sends it to the extension, once at startup and
again on every theme change. Change either theme's `--bg` in `residual.css` and
the frame follows.
It holds inside the editor as well: Monaco's theme is built by reading these
custom properties, and rebuilt whenever the resolved theme changes — see
*The editor*, below.

**Colours are hex, alpha included (`#669cff24`), never `rgba()`, in
`residual.css`.** A token is not only read by CSS — the extension parses one to
paint the frame and Monaco parses several to build its theme, both via
`getComputedStyle()` against the custom property, and neither speaks `rgba()`.
`tokens.ts`'s own exports are `var()` strings now, not hex — see
`docs/decisions.md`, *Light theme: tokens became `var()` references* — but the
rule against `rgba()` still binds wherever a colour is actually written down.

## Lineage

Radix Colors, dark and light. Nearly every value in `residual.css` is a Radix
step: `#111113` is dark slate-1, `#FCFCFD` is light slate-1, `#E5484D` is red-9
in both. Values sampled from the reference are used verbatim where they differ
...
Three choices — Dark, Light, System — set in Preferences → Settings and
resolved to `dark`/`light` by `useResolvedTheme()` (`common/theme/`). The
resolved value is stamped on `<html data-theme>` by `useThemeAttribute`, which
is what `residual.css`'s `[data-theme='light']` block and `color-scheme`
selector key off, and it is always a concrete value — never left unset for
`system` — so `thinking-orbs`' `theme="auto"` (see the recipe table) can resolve
from the same attribute rather than needing to know the preference exists.

**Ordering, if you touch this:** `useThemeAttribute` runs in a `useLayoutEffect`
so the attribute lands before paint. Anything that reads a resolved colour back
off the DOM — Monaco's `defineTheme`, the window frame's `matchFrame` call — does
it from a plain `useEffect` instead, never a layout effect: every layout effect
in the tree, at every depth, commits before any passive effect runs, so a plain
effect is guaranteed to see the attribute regardless of where in the component
tree the two hooks happen to sit relative to each other. A layout effect reading
it back would not have that guarantee — React fires layout effects bottom-up,
so a descendant's layout effect can run *before* an ancestor's.
...
either of them is still forbidden — `--veil-sheen` and `--veil-edge` are
contrast added *on top* (white on the dark theme, black on the light one —
whichever direction reads as added rather than as a second surface underneath),
which is what makes the veil read as glass rather than as a flat wash, and they
are the only place in the system contrast is spent this way.

The veil is also the one place the app blurs anything, and the wash is
deliberately thin because of it: `--veil-blur` is what obscures, and a wash heavy
enough to do that job on its own would read as paint over the row rather than
glass on it.
...
at startup — and again on every theme change, since `--bg` itself changes — to
keep that true. If the app ever comes up wearing a band the wrong colour above
...
swatches on the Radix step-11 scale (dark or light, per theme), `--conn-slate`
the neutral default; unlike a
...
`--canvas-dot` is not an exception to rule 1 either, and it is the only *texture*
in the system. It is not a second surface: it is contrast added on top of the
one background, the `--veil-sheen` family (white on dark, black on light), and
what it means is "this area pans and zooms" — which is a real fact about the
one view that has it rather than decoration. It is grayscale, so rule 2 holds.
It sits a step above `--hover` because a 4% dot at 24px spacing is invisible,
and a grid nobody can see is worse than no grid.
...
**4. The solid accent takes `--on-accent`.** Never a hardcoded dark or white.
Dark-on-accent is the reference's signature and the dark theme's answer (blue
there, teal here); the light theme's own Radix step pairs with white instead —
see `docs/decisions.md`, *`ON_ACCENT` is a token, not a hardcoded dark*. Either
way it looks wrong the moment the wrong one is hardcoded.
...
**`color-scheme` is what colours everything the browser draws for itself** —
scrollbars, focus rings, autofill, the text-field context menu. None of them is
the app's DOM and none takes CSS; this declaration is the only thing the palette
can be told through, and it is theme-driven like every other property in
`residual.css`: `dark` on `:root`, `light` on `:root[data-theme='light']`.
...
| modal | `<Modal onClose={…}>` from `components/Modal.tsx`. The one thing that *blocks*: scrim behind, outlined card on top, never a shadow. `width` defaults to 380, which is a sentence and a pair of buttons; pass a wider one only for a card that is a *table* of rows (the shortcuts screen is 560) — a dialog wide enough for its widest row is not the same thing as a dialog that wants room, which is the `<Drawer>` above. |
| keycap | A `<Button>` carrying `fontFamily: MONO` and a fixed width, in `ShortcutsDialog`. Not a `<Badge>`: a chord is the control you press to change it, not a status, and it is in a row with a real `<Button>` — so it takes `BUTTON_H` like its neighbour rather than a height of its own. While it is recording it wears an `--accent` border and `--text-muted` prompt text, which is the same "this one" language an active tab uses, spent on the one row that is listening. |
...
| context menu | `<ContextMenu x= y= items= onClose=>` from `components/ContextMenu.tsx`. Items are data, never children: each caller writes its own labels and disabled rules, and none of them re-implements the chrome. It floats without a scrim because it does not block. Four features summon one — the tree, the grid, the tab strip and the **editor**, which turns Monaco's own off (`contextmenu: false`) and draws this instead: Monaco's is a second design system in the middle of this one, with its own surface, hover, type and radius, none of it reading a token. That is why it is a primitive and not whichever feature grew it first. |
| tooltip | `<Tooltip label="…">` from `components/Tooltip.tsx`, wrapping the one trigger it names. The floating rule, on text this time: `--bg`, 1px `--border-strong`, never a shadow, `--text-badge` (12px). Shows 300ms after the pointer (or focus) arrives, which is the whole point of it existing over the native `title` it replaces — that one waits about 1.5s and cannot be styled. `disabled` skips showing it, for a trigger with its own popup already open (`ResultsColumnsButton`'s Columns button while its checkbox list is up) — a tooltip repeating what is already on screen is noise, not help. Adopted so far only where a bar button lost its label to going icon-only (see `docs/decisions.md`); the other ~78 `title=` call sites are unmigrated and that is fine — a native title is still a title, just a slower one. |
...
| menu | The titlebar's menus are inline in `Menu`; every context menu is `<ContextMenu>` above. Floating is what earns the `BORDER_STRONG` outline rather than a shadow. Menu items track their own hover state. Four of them, left to right: **File**, **Database**, **Preferences**, **About** — and **Database** is the one that comes and goes, rendered only while a shell is on screen, because it is about the database you are looking at and the connect screen is not looking at one. Their items are named data, `useTitlebarMenus.ts`, split out of `Titlebar` for length rather than for reuse — nothing else reads it. **No item label carries a trailing `…`**, whether it opens a dialog or not — the platform idiom marks "this one asks you something first", and marking some items and not others is a distinction nobody reads in a menu this short. The macOS `NSMenu` in `scripts/macos-window-chrome.m` mirrors these labels exactly, so a label edited here is edited there too — and `ui.test.ts` asserts the File and About lists verbatim. macOS is the one place *Database* is always present: that bar is built once at launch by a process that cannot see React state, so the item is there and does nothing on the connect screen. |
| settings screen | `SettingsDialog.tsx`, the Preferences menu's second item beside *Keyboard shortcuts*. `ShortcutsDialog`'s exact shape at the plain `Modal` default (380, a sentence and one control): an `<h2>`, one `<Field label="Theme">` over a default `<Select>` (Dark/Light/System, no `searchable` — three fixed options), a **Close** in a `flex-end` footer row. Backed by `useStringSetting`, the second shape beside `useBooleanSetting` in `settingsSlice.ts` — a hook per *shape*, not per *key*, same rule that hook's own doc comment states. Where the next preference (language) arrives beside this one, per the backlog. |
...
| select | `<Select>` from `components/Select.tsx` — a trigger plus a floating listbox, drawn as the native element was. **Options are data, not children** (`options={[{ value, label, disabled? }]}` + `onSelect`), the call `ContextMenu` already makes. The popup follows the floating rule: `--bg`, 1px `--border-strong`, never a shadow; the chosen row wears `--selected` + `--accent` and the one under the cursor `--hover`. `searchable` makes **the trigger itself** typeable — **opt-in, and earn it**: a search box over four fixed options is noise, so only the database picker asks for one. `variant="bare"` is the chrome form: no box at rest, semibold, 24px tall, `--text-badge` (12px) rather than the 13px `base` it otherwise inherits — it sits in a bar directly over 12px tree rows, and the mismatch is what "bare" is for — growing a 1px `--border-strong` outline on hover and while open, **deliberately not on focus**: `close()` refocuses the trigger after a pick, so a focus-triggered box would stay lit long after the pointer left it. A keyboard-focused one still rings via the global `:focus-visible` outline — that is the focus affordance here, this box is the hover one. Use it where the select *names* what you are looking at rather than collecting a value. **A control in a bar must be shorter than the bar** — `box-sizing: border-box` means a 32px strip with a 1px rule has 31px of room, so an input-height control overflows it and its box lands on the divider. `variant="attached"` is the third form and the one with no appearance of its own: no border, no background, no width, and a caret in `currentColor` rather than the muted grey the other two use, because that grey is illegible on an accent fill. It is for a trigger that is a *segment of another control* — the database half of the Run button — so everything it looks like arrives through `style` from the host. `open`/`onOpenChange` make the popup controlled, for a picker something other than its own trigger opens (`Ctrl+D`); pass both or neither, since a controlled one with no handler can never be dismissed. `caretOnly` renders the affordance and no value — for a picker whose value is already stated somewhere better, which is the Run button's. It is the one case where **search moves into the popup**: the "no field above the list" rule argues that the trigger already *is* the field's slot, and a caret has no slot to reuse, so the box sits at the top of the panel above a 1px `--border` divider. The popup also takes a 200px floor there, since matching a ~20px caret would make a sliver. `align="end"` right-aligns the popup to the trigger so the list opens *leftward* — for a trigger near the right edge of its pane, where the default left alignment grows the list away from the pane it belongs to and, in a split, unfurls it across the other one. |
...
| tab strip | Inline in TabStrip. `TAB_H` (32px). Three kinds, told apart by their glyph alone: `TableIcon` for a grid tab, `DiagramIcon` for a diagram, `QueryIcon` for an editor. A tab is a row holding a `<button data-testid="tab-pick">` plus `<button data-testid="tab-close">`, because a `<button>` cannot contain a `<button>`. Divided by 1px rules, never by a surface. Active tab tracked via `aria-selected`. A tab holding edits it has not saved back to its query draws a 7px `--text-muted` dot (`data-testid="tab-unsaved"`) **inside the close button, in place of the ×**, swapping back to the × while that button is hovered — VS Code's idiom, and the reason the mark costs no width of its own beside the label. Grayscale, like every other state in this bar. Its slot is shown whether or not the tab is active, unlike a plain close: the dot is a *state* and not an action revealed on hover, and it is the only acknowledgement a silent Ctrl+S gives. The swap is keyed on hovering the button, never the tab — at tab level the dot disappears the moment the pointer touches the tab anywhere. The strip takes `flex: 1` inside the row it shares with the saved-queries button. |
| tab drop zone | `TabDropZone` in Shell — `data-testid="dock-zone"` for the half-pane one that opens a split, `pane-drop-primary`/`pane-drop-secondary` for the whole-body ones that move a tab between two open panes. An absolutely-positioned overlay starting below the tab strip (`top: TAB_H`), shown only while a tab is being dragged. The half-pane form carries a **1px dashed `--border-strong`** left edge at rest — because a target nobody can see before hovering it is one nobody finds, and *dashed* is what says provisional: a solid accent edge standing by through every drag reads as something already happening. It stays grayscale until it is the zone being dropped on, when the edge goes `--accent` and the zone fills `--selected` (accent at 14%), the system's one existing word for "this one". The whole-body form shows nothing at rest — by then there are two panes on screen and the gesture is to drop on the other one — and takes the same dashed accent as an outline when hovered. Never a shadow, never raised: it is a region of the existing surface being called out, not a panel floated above it. **It carries a `z-index` (20)**, above the result grid's own sticky header and gutter (`z-index: 1`/`2`) and well below the 50-tier floating layer that must still cover it: a positioned element with no z-index of its own paints below those sticky cells however late it comes in the DOM, which shipped once as a drop zone that was dead over exactly the part of the grid a tab gets dragged onto. |
| saved queries popup | Inline in `features/queries/SavedQueriesButton.tsx`. The button is a `TAB_H` square at the trailing end of the strip's row, with the same 1px left and bottom rules `tab-new` carries, going `--accent` while its list is open. The list follows the floating rule — `--bg`, 1px `--border-strong`, no shadow — anchored `top: TAB_H, right: 4`, **inset rather than flush**: this is the last thing before the window's own edge, and `right: 0` puts the popup's outline on the frame. It dismisses itself on an outside `pointerdown` or Escape, the listeners `ContextMenu` already keeps. Each row is a name that opens it plus a delete that follows the reveal-on-hover rules below — **armed by a first click and committed by a second on that same button**, wearing `RED_TEXT` on `RED_BG` with a `RED` border while armed, which is `SavedConnectionList`'s trash can exactly. Not a `Yes`/`No` pair: that is a second menu for one decision, and this row is already inside a popup. An armed row counts as hovered, or the control you are being asked to confirm vanishes the moment the pointer drifts off it. Not a `<ContextMenu>` either: its items are labels, and these rows carry a second control and an empty state. |
| run group | Inline in `EditorPane`, and in `FilterBar` at that bar's 22px control height as the **search group**. An accent-filled flex box with the rounded ends, holding a primary `<Button>` and a `<Select variant="attached" caretOnly>` that each draw no box of their own. The divider between them is 1px of `color-mix(in srgb, currentColor 35%, transparent)` — the "structure from borders" rule applied *inside* a filled control, where a `--border` grey would read as a gap between two buttons rather than as one shape. **The attached half is the caret and nothing else**: the database's name is stated elsewhere in the same pane — the editor toolbar's own label at the far left, the results bar's leading word on a grid tab — because a second piece of high-contrast content inside the loudest control on screen makes it shout. The `attached` variant carries `color: inherit` — that is what makes the arrow exactly the same white as the *Run* or *Search* label beside it, rather than `--text` sitting on an accent fill. One idiom and two instances, because they are one control: the loudest thing in the bar, with the database it acts on hung off its side. See `docs/decisions.md`. |
| toolbar database label | Inline in `EditorPane`, first thing in the bar, before the spacer. The name of the database the tab runs against, at `TEXT_LABEL` (11px) in `--text-muted` — the one place that size is used **without** the uppercase and letter-spacing the token is otherwise for, because it is a name the server gave us and casing anything the server said is what the value rules forbid. A label, not a control: the caret on Run, the sidebar picker and `Ctrl+D` are the three ways to change it. A grid tab has no toolbar, so its equivalent is the **first word of the results bar** — same rule, same reason, in the only bar that tab has. |
...
| sidebar filter | Inline in Sidebar, a second `TAB_H` strip directly beneath the header with its own 1px bottom rule, holding one `<Input variant="bare">`, then a ghost refresh `<Button>`, then the tree/tab sync toggle `<Button>` last. Refresh before the toggle, not after: the header above ends in `[refresh][collapse]`, so putting the *other* toggle-like control last here is what lands both refresh icons in the same column instead of one sitting a button-width off. Boxless at rest like the picker above it, so the two strips read as one continuous head rather than as a title with a form stuck under it; its placeholder is what says it can be typed into. |
| pressed icon toggle | A ghost icon `<Button>` carrying `aria-pressed` and, when set, `color: ACCENT` — the sidebar's tree/tab sync toggle is the one of these. The glyph never changes, unlike the fold/unfold and lock/unlock pairs: those name two *actions* and show the one clicking would take, while this names one state being on or off, and `--accent` is already the system's word for "this one". The colour goes on the button, so the glyph inherits it — the icon rule above, not an exception to it. |
| connection rail | Inline in ConnectionRail. A full-width horizontal bar (`RAIL_H`, which *is* `TAB_H`) across the top of the shell, above the sidebar and the tabs, with a 1px bottom rule like every other bar. Open connections are grouped by workspace, and a group is one row: the workspace's name and glyph in plain `--text-muted`, then its chips beside them — there is no room to stack a heading over the chips at this height, and groups are told apart by the 1px rule between them. **The heading itself carries no hue** — a workspace has none of its own, only its connections do, so there is nothing to paint it with; see `docs/decisions.md`. Each chip resolves its own tint from `connectionColor(c.color)` (a `var(--conn-*)` reference from `tokens.ts`) and spends it directly in inline styles. Every chip hue is a *blend toward `--bg`*, at ratios `chipTints(theme)` names: a chip's border at 0.3, its wash at 0.07, and the active chip's fill at 0.72 on the dark theme — full strength (1) on the light one, since the active chip pairs its fill with `--bg` text (`ON_ACCENT`'s idiom, spent on a connection's colour rather than the app's accent) and that pairing wants the tint at its own Radix step, not blended toward a near-white ground; see `docs/decisions.md`. Those numbers are otherwise the whole of the rail's volume — it sits above the editor and the results, and full-strength tint everywhere pulls the eye off both. |
...
| veiled row | `AwsSignInVeil` in `features/connections/AwsSignIn.tsx`, `data-testid="saved-blocked"`. A row that is *there but not usable yet* — a saved connection whose AWS profile has not signed in. An absolutely positioned pane across the row's click target, revealed on hover/focus, carrying the one action that unblocks it. **Frosted glass, and the frost is masked** — the whole of how it works: a `blur(--veil-blur) saturate(1.3)` backdrop with `--veil` graded to `--veil-deep`, a `--veil-sheen` highlight down the top and a 1px `--veil-edge` hairline top and bottom, all of it behind a `mask-image` that is deepest at the leading edge, under the chip, and thins away to nothing across the row. The mask is what makes a real blur affordable: the frost thins out well before the row's trailing end, so the server line stays legible and still says which connection is being asked about. **A mask, never a `clip-path`** — a clip takes the uncovered half out of the hit target too, and the whole pane is the click. **The glass and the label are separate elements**: the frost is an `aria-hidden` layer that carries every effect and no text, the label rides above it unmasked. **The reveal fades on the frost itself, not on the pane** — an *ancestor* mid-opacity isolates the backdrop, so a fade one level up leaves the blur sampling an empty group until it lands and then snaps in. **The label carries a ground of its own**, capped at 72% of the pane: it is opaque wherever it falls, so a wide one reaches the thin end of the frost and lands on row text that is still sharp. What that ground looks like is the shape grammar and nothing else — **the sign-in is a button**, so it is the primary to the letter (solid `--accent`, `--on-accent` text, 600, `--radius` at `--button-h`, which also lines it up with the row's own *Edit*); **the missing-profile line is a state**, so it is a pill, filled `--veil-deep` and muted with no edge. Neither is an actual `<Button>` or `<Badge>`: the pane *is* the `<button>`, and one cannot contain the other. The sign-in draws no hover state either, unlike a real `<Button>` — the pane is only up while it is already hovered or focused, so a hover fill would be the only fill anyone ever saw. **The whole pane is the target**, a `<button>` rather than a button floating inside a `<div>` — the pane is already row-width, and a control centred in it leaves nine tenths of an obviously interactive surface doing nothing. **The chip sits at the leading edge**, where the frost is deepest and where the eye starts. **Dimming the row at rest is what carries the state** (`opacity: 0.5` on the pick target) — with the pane revealed only on hover, a blocked row would otherwise look live right up to the click that does nothing. `pointerEvents` tracks the reveal, per the hover-reveal rules below, or the invisible pane eats the row's clicks. Cover only what is refused: *Edit* and *Delete* stay reachable, since editing is one of the ways out. Reach for it when an action is *blocked pending one specific step* — never for loading (that is `<Skeleton>`), never for a permanent state (that is a disabled control with a reason). |
...
| status bar | Inline in StatusBar. `STATUSBAR_H` (26px), a 1px top rule like every other bar, one background. Grayscale: it carries the read-only lock (a closed vs open lock, shape not hue) and the active connection's environment as a plain word. **The assistant's provider is the one segment here that is not about a connection** (`AssistantStatus`, `data-testid="statusbar-assistant"`): the assistant's glyph, the name of whoever's key is stored (*Claude*, *ChatGPT*), and a 5px `--accent` dot while a turn is running — the only hue in the bar, and it means "happening now" rather than naming a status. The **provider**, never the model: which model answers is per conversation and already stated in that tab's composer, while who is being billed is true of the whole window. Clicking opens a `<ContextMenu>` rather than acting: throwing a key away is not a thing to do by mis-clicking a status bar, which is the tab strip's close-button reasoning applied to the quietest strip in the app. |
| canvas tab | `RelationshipDiagram` in `features/diagram/`, `data-testid="diagram"`. A tab whose body is a pannable, zoomable surface rather than a grid or an editor — it fills the pane's content row, exactly where `ResultsTable` otherwise goes. Its head is an ordinary `TAB_H` bar with a 1px bottom rule like every other, and it is the **editor toolbar's shape**: the tab's database at the far left, a count in `TEXT_FAINT`, then ghost `BUTTON_H_BAR` controls pushed right by a `flex: 1` spacer — zoom out, the zoom level (which resets it), zoom in, and **refresh last**, the sidebar header's 24px icon button with the same `spin` class while a read is in flight. The database is a `<Select variant="bare" searchable>`, the sidebar header's picker rather than the editor's caret: there is no filled primary control in this bar to attach a caret to, and this select *names what you are looking at*, which is exactly what `bare` is for. No close button — the tab's × is the close, and a second one inside would be two controls for one act. The canvas takes `--canvas-dot` as a `radial-gradient` at `GRID_SPACING`, on the *sized* element rather than the scaled one, so the spacing follows the zoom while each dot stays 1px; the element is `max(drawing, 100%)` in both axes or the dots stop short of the pane's edges, and the drawing it is sized to is measured from where the nodes have been *dragged* to, not from where they were laid out — otherwise a node moved past the edge sits on bare background outside its own canvas. Ctrl+wheel zooms via a native `passive: false` listener (React's is passive, so `preventDefault` there does nothing and the webview zooms itself). |
| diagram node | Inline in `RelationshipDiagram`, `data-testid="diagram-node"`. The card recipe at `RADIUS_LG` with a 1px `BORDER_STRONG` outline, and a `<button>` rather than a `<div>` because the whole box opens that table — the same gesture as its tree row. A `ROW_H_DENSE` header (`TableIcon` + the relation's label in `MONO`) over a 1px `--border` rule, then one `ROW_H_TIGHT` row per column: the tree's expanded-columns row exactly, including its two-stage shrink — the name keeps the default flex weight and the type a far higher one, so a long type gives up its width before the name truncates. `KeyIcon` marks a primary key and `ForeignKeyIcon` a foreign key, both `TEXT_MUTED`: shape, not hue, like every other mark in the chrome. Hovered, it takes an `ACCENT` border and a `SELECTED` header — the system's one word for "this one", the same as an active tab — and its own lines light `ACCENT` while every other line drops to 0.35 opacity. Being dragged looks the same and is `cursor: grabbing`, plus a `z-index` lift so it never slides behind a taller neighbour. It carries `touch-action: none` and `user-select: none`, which are not tidiness: without the first the browser claims the gesture as a pan and fires `pointercancel` mid-drag, and its `background` is `--bg` rather than transparent so the dotted canvas does not read through the rows. The lines themselves are 1px `BORDER_STRONG` paths in a single `<svg>` spanning the canvas with `pointerEvents: none`, so the nodes stay draggable through it; the arrowhead is an SVG `<marker>`, and there are **two** of them because a marker cannot inherit the stroke of the path that references it. |
| Assistant tab | `features/assistant/AssistantPanel.tsx`, `data-testid="assistant-panel"`. The body of a tab, filling the pane where a result grid or a diagram otherwise goes. **A `TAB_H` bar holds only what acts on this conversation** — the glyph, the word *Assistant*, a `HistoryIcon` button opening the past conversations (`data-testid="ai-history-open"`), and a ghost `BUTTON_H_BAR` **new conversation** button (`data-testid="ai-new-conversation"`). That second one is `NewConversationIcon`, a `+`, and **not a bin** — nothing is destroyed by it, since the thread being left keeps its stored row and is in the popup beside it. It was a `DeleteIcon` once, describing a reducer that predated conversations being kept; a glyph saying *delete* over behaviour saying *keep* is a mismatch the user has to discover by risking something. The history is offered whether or not this tab holds anything, since reaching a past conversation is most wanted from an empty one, while the `+` appears only once there is a conversation to leave; the history button is disabled while a turn is in flight, because reopening would swap the thread out from under a loop still writing into it. Its popup is the saved-queries panel's shape and rules — outlined, never raised, dismissed by a pointer outside its root or Escape, each row a name with a `TEXT_FAINT` relative time and a delete armed by one click and committed by a second. The provider and its key are the status bar's; the model and mode `<Select variant="bare">`s live in a **second row under the composer**, VS Code's shape. That split is a rule this bar taught rather than a preference: *a 32px strip fits a title and one or two icon buttons, and a control put there beyond that is a control someone will not find*. Six of them shipped once — icon, account, model, mode, clear, sign out — every one `flex: none`, totalling ~610px, and in an ordinary pane the last two were simply pushed off the right edge. Reported as "I see the account, but not the sign out", which is exactly what an overflowing flex row looks like: the first item is fine and the last one is gone. Two consequences worth keeping: **in a bar, exactly one thing may be `flex: 1`, and it must be the least important**, since a truncated label still reads while a missing button cannot be pressed. And **a `<Select>` needs a width of its own in a flex row**, since its base style carries `width: 100%` and it is otherwise what collapses; a `searchable` one is worse, because its trigger is an `<input>` whose *placeholder* carries the label, so a long value is clipped from the end. `align="end"` for the reason that variant documents: the trigger is near the window's trailing edge, and a left-aligned popup would unfurl back across the editor. A user message is an outlined bubble at `RADIUS_LG` pushed to the trailing edge; an answer is plain prose with no bubble at all, because one side of a conversation being unmarked is what tells the two apart without spending a surface on either. An answer is rendered by `Markdown.tsx`: headings at one step of weight and size rather than six (a six-level hierarchy in a panel this narrow is typography for a document nobody is reading), inline code on `--hover` at `MONO`, lists, a block quote behind a 2px `--border-strong` rule, a `--border` `<hr>`, and tables whose cells are 1px `--border` and whose wrapper is its own `overflow-x: auto` — a ten-column answer must not push the conversation sideways. A fenced code block is a 1px `--border` box in `MONO`. It is a hand-rolled subset and not a markdown dependency: no images, no raw HTML, and a link renders as its label plus a `--text-faint` URL rather than as an anchor, because a one-click path from model output to a browser is a bigger door than a chat panel needs. A tool call is a collapsed row: disclosure caret, `ToolIcon`, the tool's name in `MONO`, then its target in `TEXT_FAINT`, with a `<Badge>` only when it was declined or failed. **Expanding it shows what was sent and what came back**, indented behind a 1px `--border` rule, each capped rather than scrolled — a `getRelationships` answer is tens of kilobytes of JSON and a box that long buries the conversation it annotates. The disclosure has to actually disclose: it shipped once toggling a state nothing rendered, which is a caret that turns and does nothing. The approval card is the one thing here that draws attention — 1px `--accent` and a `--selected` fill, the system's existing word for "this one", the same language an active tab uses — and it is a card, never a `<Modal>`: it belongs in the transcript where the call it is about happened, and a dialog would take the thread off screen at exactly the moment you want to read what led to it. The composer is a `<textarea>` rather than an `<Input>` so Shift+Enter can break a line while Enter sends, and its send button takes `alignSelf: stretch` rather than `BUTTON_H` — the two are one input, and a 30px control parked at the corner of a two-row box reads as two. **The empty state is centred with its glyph above it** (`data-testid="ai-empty"`), not two sentences in the top-left: an empty pane whose only content hugs the corner reads as broken rather than as waiting. No card around it — one background, structure from spacing and weight. Before a key exists the pane holds `Connect.tsx` (`data-testid="ai-connect"`) instead: prose, a `<Field>`ed provider `<Select>`, a `type="password"` `<Input>`, and a primary *Connect* beside a ghost *Get a … key*. A form in a pane, so it takes the connect screen's plain vertical stack at `GAP_XL` padding and no card — and the key is a password field not because anyone is shoulder-surfing a local editor but because that is what keeps it out of a screenshot or a screen share, which is how these actually leak. Whatever the provider said about a refusal is shown verbatim in a `<Callout>`; *this key is not funded* and *this is not a key* are two different errands. |
...
wear this one. `features/editor/monaco.ts` is where that happens: `defineTheme(base)`
reads the CSS custom properties and (re)registers the theme, called once at
mount and again from a plain `useEffect` whenever the resolved theme changes —
re-defining an already-registered theme updates every live editor, so no editor
is ever torn down for a theme switch. `base` (`'vs-dark'` or `'vs'`, from
`monacoBase(theme)`) travels with it: it is what resolves every widget colour
the `colors` map below leaves unset, so a light theme built on `vs-dark` would
come up with a dark scrollbar and a dark suggest-widget shadow no rule here
names. Three places the rules had to be enforced against Monaco's defaults, all
of which are how it stays part of the app rather than a panel embedded in it:
...
- **The suggest widget is the find widget's situation exactly.** It floats, so it
  ...
  list at all. Verified against the live widget rather than assumed, on the
  dark theme: `box-shadow: none`, background `#111113`, border `#363a3f` at 1px.
...
palette (later given a Radix light counterpart, on the same lineage, when the
app grew a theme setting); accent-with-`--on-accent`-text primaries (blue in the
reference, teal here); the badge recipe (step-3 bg + step-11

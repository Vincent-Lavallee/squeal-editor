...
import {
    ...
    connectionColor,
} from '../../common/icons/connectionColors.ts';
...
const swatchTile: React.CSSProperties = {
    display: 'grid',
    placeItems: 'center',
    width: 32,
    height: 32,
...
export default function ConnectionForm({
    ...
    const [form, setForm] = useState<FormState>(() =>
        initialState(initial, environments[0]?.name ?? ''),
    );
    ...
    const requiredHint = (field: RequiredField): React.ReactNode =>
        invalid(field) ? REQUIRED : undefined;
          ...
          last, every answer above it was given without knowing that. */}
            <Field label="Engine" htmlFor="type">
                <Select
                    id="type"
                    value={form.type}
                    onSelect={(value) => setEngine(value as EngineType)}
          ...
          moves. */}
                            ...
                            return (
                                <label
                                    ...
                                    style={{
                                        ...swatchTile,
                                        ...(on
                                            ? { borderColor: t.ACCENT, background: t.SELECTED }
                                            : {}),
                                    }}
                                >
                                    <input
                ...
                <div style={{ display: 'flex', gap: 10 }}>
                    ...
                    <Field label="Color">
                        <button
                            ...
                            style={swatchTile}
                            aria-expanded={false}
                            aria-label={`Color: ${form.color}. Choose another`}
                            onClick={() => setPicking(true)}
                        >
                            <span
                                aria-hidden="true"
                                style={{ ...swatchDot, background: connectionColor(form.color) }}
                            />
                        </button>
                    </Field>
                </div>
            ...
            {fileBased ? (
                    ...
                    <div style={{ display: 'flex', gap: t.GAP_SM }}>
                        <div style={{ flex: 1 }}>
                            <Input
                                ...
                                aria-invalid={invalid('database') || undefined}
                                style={invalidBox('database')}
                ...
                already have the path, that is the shorter route to it. */}
                        <Button onClick={() => void browseForFile()} disabled={busy}>
                            Browse…
                        </Button>
                    </div>
                </Field>
                ...
                <>
                        ...
                        <div style={{ flex: 1 }}>
                            <Field label="Host" htmlFor="host" hint={requiredHint('host')}>
                                <Input
                                    id="host"
                                    value={form.host}
                                    aria-invalid={invalid('host') || undefined}
                                    style={invalidBox('host')}
                    ...
                    <Field
                        ...
                        htmlFor="database"
                        hint={
                            form.type === 'postgres' ? <span>(default: postgres)</span> : OPTIONAL
                        }
                    >
                        <Input
                            id="database"
                            ...
                            <div style={{ display: 'flex', gap: 10 }}>
                                <div style={{ flex: 1 }}>
                                    <Field
                                        label="AWS profile"
                                        htmlFor="awsProfile"
                                        hint={requiredHint('awsProfile')}
                                    >
                                        <Input
                                            id="awsProfile"
                                            value={form.awsProfile}
                                            placeholder="default"
                                            aria-invalid={invalid('awsProfile') || undefined}
                                            style={invalidBox('awsProfile')}
                                ...
                                <div style={{ flex: 1 }}>
                                    <Field
                                        label="Region"
                                        htmlFor="awsRegion"
                                        hint={requiredHint('awsRegion')}
                                    >
                                        <Input
                                            id="awsRegion"
                                            value={form.awsRegion}
                                            placeholder="us-east-1"
                                            aria-invalid={invalid('awsRegion') || undefined}
                                            style={invalidBox('awsRegion')}
                  ...
                  of this. */}
                        ...
                        <Field label="Password" hint={OPTIONAL} htmlFor="password">
                            <Input
                                ...
                                placeholder={
                                    mode === 'edit' && initial?.hasPassword ? 'unchanged' : ''
                                }
                                onChange={(e) => {
            ...
            >
                {onAbortConnect ? (
                    <>
                        ...
                        >
                            Connecting
                            {connectingElapsed === undefined
                                ? '…'
                                : ` for ${connectingElapsed.toFixed(1)}s…`}
                        </Button>
                    </>
                ) : (
                    <>
                ...
                it is, since fixing a field and trying again is the whole point. */}
                        <Button
                            ...
                            onClick={() => test(serverConfig(form, iam), testPassword)}
                            disabled={!testable}
                        >
            ...
            {serverVersion !== null && (
                <div data-testid="connect-test-result">
                    <Callout tone="success">
                        Connected to {engine.label} {serverVersion}
                    </Callout>
                </div>
            )}
            {testError && (

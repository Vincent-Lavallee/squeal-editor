...
function screenSubtitle(screen: Screen): string {
  switch (screen.view) {
    case 'workspaces': return 'Choose a workspace.';
    case 'workspaceNew': return 'Name a new workspace.';
    case 'workspaceEdit': return 'Rename this workspace, or give it another mark.';
    case 'list': return 'Pick a connection, or add one.';
    case 'new': return 'Describe the server you want to reach.';
    case 'edit': return 'Change what this connection points at.';
    case 'password': return 'This connection did not save its password.';
  }
}
...
export default function ConnectScreen({ onCancel }: Props) {
  ...
  function go(next: Screen | null): void {
    saved.dismissError(); workspaces.dismissError(); session.dismissError(); awsSignIn.clear();
    ...
    setConnectingId(null); setDraftRowId(null); setScreen(next);
  }
  ...
  async function submitNew(workspaceId: string, values: FormValues): Promise<void> {
    ...
    try { row = await saved.save({ id: draftRowId ?? undefined, workspaceId, name: values.name, config: values.config, environment: values.environment, readOnly: values.readOnly, password: passwordUpdate(values, 'new'), color: values.color }); }
    catch { return; }
  ...
  const attemptedConnection = connectingId ? saved.connections.find((c) => c.id === connectingId) : undefined;
  /**
   ...
   */
  const failedIamProfile = session.awsCredentialsFailed ? (attemptedConnection?.config.iam?.profile ?? null) : null;
  ...
  function renderScreen() {
    ...
    switch (resolved.view) {
      ...
      case 'new': {
        ...
        return <ConnectionForm mode="new" environments={environments.environments} busy={busy} onSubmit={(values) => void submitNew(resolved.workspaceId, values)} onCancel={populated ? () => go({ view: 'list', workspaceId: resolved.workspaceId }) : () => go({ view: 'workspaces' })} onAbortConnect={session.connecting ? abortConnect : undefined} connectingElapsed={connectingElapsed} />;
      }
      case 'list':
        return <SavedConnectionList workspace={workspaceById(resolved.workspaceId)!} connections={saved.connections.filter((c) => c.workspaceId === resolved.workspaceId)} environments={environments.environments} connectingId={session.connecting ? connectingId : null} connectingPhase={session.connectingPhase} openIds={openIds} busy={busy} onPick={(connection) => void pick(connection)} onEdit={(connection) => go({ view: 'edit', connection })} onDelete={(id) => { go({ view: 'list', workspaceId: resolved.workspaceId }); saved.remove(id); }} onNew={() => go({ view: 'new', workspaceId: resolved.workspaceId })} onBack={() => go({ view: 'workspaces' })} />;
    }
  }
      ...
      <div style={{ background: t.BG, border: `1px solid ${t.BORDER_STRONG}`, borderRadius: t.RADIUS_LG, padding: t.GAP_XL, width: 420 }}>
        ...
        {error && (
          error === 'Cancelled.'
            ...
            : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: t.GAP_SM, marginTop: t.GAP_LG }}>
                ...
                {failedIamProfile && connectingId && (
                  <AwsSignInButton profile={failedIamProfile} disabled={busy}
                    onSignedIn={() => { session.dismissError(); void session.connectSaved(connectingId); }} />
                )}
              </div>
            )
        )}

 ...
 */
const FROST_RAMP = 'linear-gradient(90deg, transparent 0%, #000 6%, #000 40%, #000000a6 64%, transparent 100%)';
    ...
    return (
      <div data-testid="saved-blocked" title={reason ?? undefined} style={pane}>
        ...
        {/* Filled but not outlined, and muted: nothing here is clickable, so
            ...
            the pane's `title`. */}
        <span style={{ ...chip, padding: '4px 10px', borderRadius: t.RADIUS_PILL, background: t.VEIL_DEEP, color: t.TEXT_MUTED, opacity: shown ? 1 : 0 }}>
          Profile not set up
        </span>
      </div>
    );

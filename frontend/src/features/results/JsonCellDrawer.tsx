...
const header: React.CSSProperties = { display: 'flex', alignItems: 'center', gap: t.GAP_SM, flex: 'none', padding: `0 ${t.GAP_LG}px`, height: t.TAB_H, borderBottom: `1px solid ${t.BORDER}` };
const errorBar: React.CSSProperties = { flex: 'none', padding: `${t.GAP_SM}px ${t.GAP_LG}px`, borderBottom: `1px solid ${t.BORDER}`, background: t.RED_BG, color: t.RED_TEXT, fontSize: t.TEXT_BADGE, fontFamily: t.MONO, whiteSpace: 'pre-wrap', wordBreak: 'break-word' };
const footer: React.CSSProperties = { display: 'flex', alignItems: 'center', gap: t.GAP_XS, flex: 'none', padding: t.GAP_SM, borderTop: `1px solid ${t.BORDER}` };
 ...
 */
export default function JsonCellDrawer({ column, dataType, initial, canNull, onCommit, onNull, onCancel }: Props) {
  ...
  const save = () => { if (valid && editorRef.current) onCommit(editorRef.current.getValue()); };

  return (
    <Drawer onClose={onCancel}>
      <div style={header}>
        <Mono>{column}</Mono>
        {dataType && <span style={{ color: t.TEXT_FAINT, fontSize: t.TEXT_BADGE }}>{dataType}</span>}
      </div>

      <div style={{ flex: 1, minHeight: 0 }} ref={host} />

      {parseError && <div data-testid="json-drawer-error" style={errorBar}>{parseError}</div>}

      <div style={footer}>
        {canNull && <Button variant="ghost" onClick={onNull}>Set NULL</Button>}
        <Button variant="ghost" onClick={format} disabled={!valid}>Format</Button>
        <div style={{ display: 'flex', gap: t.GAP_XS, marginLeft: 'auto' }}>
          <Button onClick={onCancel}>Cancel</Button>
          <Button variant="primary" data-testid="json-drawer-save" onClick={save} disabled={!valid}>Save</Button>
        </div>
      </div>
    </Drawer>
  );

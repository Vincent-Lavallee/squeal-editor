...
import { useDisplayResult } from '../../grid/hooks/useDisplayResult.ts';
...
export function useResultsCore(tab: Tab | null) {
    ...
    // column order, with its hidden columns dropped -- see `useDisplayResult`.
    const order = activeTabId ? view.columnOrderFor(activeTabId) : [];
    const hidden = activeTabId ? view.hiddenColumnsFor(activeTabId) : NO_HIDDEN;
    const { full: fullResult, result } = useDisplayResult(rawPart.result, order, hidden);

...
export default function Sidebar({
    ...
    const {
        ...
        firstLoad,
        error,
    } = useExplorer(shownDatabase, filter);
        ...
        return (
            <div key={key} data-testid="tree-item">
                <div
                    ...
                    onContextMenu={(e) => {
                        e.preventDefault();
                        setMenu({ kind: 'table', table, x: e.clientX, y: e.clientY });
                    }}
                >
                    <button
                    ...
                    <button
                        ...
                        title={`${key} — click to browse`}
                    >
                        {table.kind === 'view' ? (
                            <ViewIcon
                                style={{ ...iconSvg, color: t.TEXT_MUTED }}
                                aria-hidden="true"
                            />
                        ) : (
                            <TableIcon
                ...
                {open && (
                    <Columns columns={columnsFor(db, relationOf(table))} indented={indented} />
                )}
                {open && (
                    <Triggers
                        triggers={triggersFor(db, table.name)}
                        table={table.name}
                        schema={table.schema}
                        indented={indented}
                        onLoadTriggers={() => loadTableTriggers(db, table.name, table.schema)}
                        onShowDefinition={(trigger) =>
                            onShowTriggerDefinition(db, table.name, trigger, table.schema)
                        }
                        onContextMenu={(trigger, x, y) =>
                            setMenu({
                                kind: 'trigger',
                                trigger,
                                table: table.name,
        ...
        return (
            ...
            >
                <div
                    ...
                    onContextMenu={(e) => {
                        e.preventDefault();
                        setMenu({ kind: 'function', func, x: e.clientX, y: e.clientY });
                    }}
                >
                    <button
        ...
        return (
            <div data-testid="tree-functions">
                <button
                    ...
                    onClick={() => toggleFunctions(schema)}
                    aria-expanded={open}
        ...
        >
            ...
            >
                ...
                <Button
                    ...
                    aria-label={
                        synced
                            ? 'Stop the tree following the tab'
                            : "Keep the tree on the tab's database"
                    }
                    aria-pressed={synced}
            ...
            >
                        ...
                        return (
                            <div key={schema} data-testid="tree-schema">
                                ...
                                {open &&
                                    ...
                                    renderFunctions(schema, schemaFunctions, database, true)}
                            </div>
                        );
                    })}
                ...
                {database &&
                    !grouped &&
                    unpinned?.map((table) => renderRow(table, database, false))}
            ...
            {dropping && database && (
                <DropTableConfirm
                    table={dropping}
                    onConfirm={async () => {
                        await dropTable(database, relationOf(dropping), dropping.kind);
                        setDropping(null);
                    }}

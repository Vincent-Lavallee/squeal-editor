...
/**
 * How much room a set of nodes needs, **where they are actually drawn**.
 *
 ...
 *
 * The other half of that fix is not here: a node may not be dragged to a
 * *negative* coordinate at all, because no scroll container can reach one. See
 * `RelationshipDiagram`.
 */
export function extentOf(nodes: DiagramNode[]): { width: number; height: number } {
  let width = 0;
  let height = 0;
  ...
  return { width: width + CANVAS_PAD, height: height + CANVAS_PAD };
}

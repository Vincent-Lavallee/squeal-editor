...
function resolveConnection(args: Record<string, unknown>, ctx: ToolContext) {
  ...
  const active = state.session.activeConnectionId ? state.session.connections[state.session.activeConnectionId] : null;
  if (!active) throw new Error('No connection is open.');
  return active;
}

function resolveDatabase(args: Record<string, unknown>, ctx: ToolContext, connectionId: string): string {
...
function requireDatabase(name: string, ctx: ToolContext, connectionId: string): string {
  ...
  if (known.length && !known.includes(name)) {
    throw new Error(`That connection has no database called "${name}". It has: ${known.join(', ')}.`);
  }
  return name;
}
...
const connectionArg = {
  connectionId: { type: 'string', description: 'Which open connection, from getConnections. Omit for the one in front.' },
} as const;

const databaseArg = {
  database: { type: 'string', description: 'Which database. Omit for the one the user is working in.' },
} as const;
...
export const TOOLS: Tool[] = [
  /* -- Schema. Four jobs, and each description says when not to use it. ---- */
  {
    def: {
      ...
      parameters: {
        type: 'object',
        properties: { ...connectionArg, ...databaseArg, search: { type: 'string', description: 'Part of a table name.' } },
        required: ['search'],
      },
    },
    ...
    def: {
      ...
      parameters: {
        ...
        properties: {
          ...
          ...databaseArg,
          table: { type: 'string' },
          schema: { type: 'string', description: 'Postgres schema. Omit for MySQL and SQLite.' },
        },
        required: ['table'],
      },
    },
  ...
  },
  {
    ...
    async run(args, ctx) {
      const connection = resolveConnection(args, ctx);
      const database = resolveDatabase(args, ctx, connection.connectionId);
      const { tables } = await call('db.relationships', { connectionId: connection.connectionId, database });
    ...
    async run(args, ctx) {
      ...
      return {
        ...
        tabs: state.tabs.tabs
          .filter((tab) => tab.connectionId === connection.connectionId && tab.kind !== 'assistant')
          .map((tab) => ({
            tabId: tab.id,
            title: tab.title,
            kind: tab.kind,
  ...
  },
  {
    ...
    async run(args, ctx) {
      ...
      return {
        ...
        database: tab.database,
        sql,
        statements: statementSpans(sql, dialect).map((span, index) => ({ index, text: span.text })),
      };
    },
  },
  {
    def: {
      ...
      description: "The text the user has highlighted in the editor, if any. Use this when they say 'this query' or 'explain this'.",
      parameters: { type: 'object', properties: {} },
    },
    ...
    async run(_args, ctx) {
      ...
      return { selected: true, tabId: selection.tabId, title: tab?.title, text: selection.text };
    },
  },
  {
    def: {
      ...
      parameters: {
        type: 'object',
        properties: { ...connectionArg, ...databaseArg, sql: { type: 'string' }, title: { type: 'string' } },
        required: ['sql'],
      },
    },
    ...
    async run(args, ctx) {
      ...
      const named = str(args.database);
      const database = named ? requireDatabase(named, ctx, connection.connectionId) : undefined;
  ...
  },
  {
    ...
    async run(args, ctx) {
      ...
      if (tab.kind !== 'editor') throw new Error('Only an editor tab can be pointed at a different database.');

      const database = requireDatabase(String(args.database), ctx, tab.connectionId);
      ctx.dispatch(databaseChanged({ connectionId: tab.connectionId, tabId: tab.id, database }));
  ...
  },
  {
    def: {
      ...
      parameters: {
        ...
        properties: {
          tabId: { type: 'string' },
          sql: { type: 'string' },
          statementIndex: { type: 'integer', description: 'From getTabContent. Omit to replace the whole tab.' },
        },
        required: ['tabId', 'sql'],
      },
    },
    ...
    async run(args, ctx) {
      ...
      return { tabId: tab.id, replaced: index === undefined ? 'the whole tab' : `statement ${index + 1}` };
    },
  },
  ...
  },
  {
    ...
    async run(args, ctx) {
      ...
      const outcome = await ctx.dispatch(runQuery({ tabId: tab.id, sql: span.text, part: index }));
      if (runQuery.rejected.match(outcome)) throw new Error(String(outcome.payload ?? 'The query failed.'));
      return { tabId: tab.id, statementIndex: index, ...describeResult(outcome.payload.result) };
    },
  },
  ...
  /* -- The one tool that carries values out. Not gated; see the header. ---- */
  {
    def: {
      ...
      parameters: {
        type: 'object',
        properties: { tabId: { type: 'string' }, maxRows: { type: 'integer', description: 'Default 50.' } },
        required: ['tabId'],
      },
    },
    target: (args, ctx) => {
    ...
    async run(args, ctx) {
      ...
      if (!part?.result) return { tabId: tab.id, rows: [], note: 'That tab has no result on screen.' };
...
export const toolByName = (name: string): Tool | undefined => TOOLS.find((tool) => tool.def.name === name);

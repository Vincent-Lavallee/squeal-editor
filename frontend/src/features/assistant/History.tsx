...
export default function History({ tabId, onOpen, disabled }: { tabId: string; onOpen: (id: string) => void; disabled: boolean }) {
  ...
  return (
    <div ref={root} style={{ position: 'relative', display: 'flex', flex: 'none' }}>
      ...
      <button data-testid="ai-history-open" aria-label="Past conversations" aria-expanded={open}
        title={disabled ? 'Finish or stop this turn first' : 'Past conversations'} disabled={disabled}
        style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: t.BUTTON_H_BAR, padding: '0 6px', border: 'none', borderRadius: t.RADIUS, background: 'none', color: open ? t.ACCENT : t.TEXT_MUTED, cursor: disabled ? 'default' : 'pointer', opacity: disabled ? 0.4 : 1 }}
        ...
        <div data-testid="ai-history-panel" style={panel} role="menu">
          {conversations.length === 0 ? (
            <p style={{ margin: 0, padding: `${t.GAP_SM}px 8px`, color: t.TEXT_FAINT, fontSize: t.TEXT_BADGE }}>
              No past conversations. The ones you have are kept here when you move on from them.
            </p>
          ) : (
            conversations.map((conversation) => {
              // Armed counts as shown: the row you are confirming must not lose
              // its own delete button when the pointer moves off it.
              const shows = hoveredId === conversation.id || confirmingId === conversation.id;
              return (
                <div data-testid="ai-history-row" key={conversation.id}
                  style={{ display: 'flex', alignItems: 'center', gap: t.GAP_XS, borderRadius: t.RADIUS, ...(shows ? { background: t.HOVER } : {}) }}
                  onMouseEnter={() => setHoveredId(conversation.id)} onMouseLeave={() => setHoveredId(null)}>
                  <button data-testid="ai-history-pick" role="menuitem" title={conversation.title}
                    style={{ display: 'flex', alignItems: 'baseline', gap: t.GAP_SM, flex: 1, minWidth: 0, padding: '6px 8px', border: 'none', background: 'none', color: t.TEXT, font: 'inherit', fontSize: t.TEXT_BODY, textAlign: 'left', cursor: 'pointer' }}
                    onClick={() => { setOpen(false); onOpen(conversation.id); }}>
                    <span style={{ flex: 1, minWidth: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                      {conversation.title}
                    </span>
                    <span style={{ flex: 'none', color: t.TEXT_FAINT, fontSize: t.TEXT_BADGE }}>{ago(conversation.updatedAt)}</span>
                  </button>
                  {/* Armed by a first click, committed by a second on the same
                      ...
                      cursor. */}
                  <button data-testid="ai-history-delete"
                    aria-label={confirmingId === conversation.id ? `Click again to delete ${conversation.title}` : `Delete ${conversation.title}`}
                    title={confirmingId === conversation.id ? 'Click again to delete' : 'Delete'}
                    style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', flex: 'none', width: 22, height: 22, marginRight: t.GAP_XS, padding: 0, border: '1px solid transparent', borderRadius: t.RADIUS, background: 'none', color: t.TEXT_MUTED, cursor: 'pointer', opacity: shows ? 1 : 0, pointerEvents: shows ? 'auto' : 'none', ...(confirmingId === conversation.id ? { color: t.RED_TEXT, background: t.RED_BG, borderColor: t.RED } : {}) }}
                    onClick={() => { if (confirmingId === conversation.id) { remove(conversation.id); setConfirmingId(null); } else setConfirmingId(conversation.id); }}>

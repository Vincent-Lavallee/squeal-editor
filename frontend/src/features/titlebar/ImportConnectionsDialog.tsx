...
export default function ImportConnectionsDialog({ onClose }: Props) {
  ...
  async function choose(): Promise<void> {
    const chosen = await Neutralino.os.showOpenDialog('Import connections', { filters: FILTERS });
  ...
  return (
    <Modal onClose={close}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: t.GAP }}>
        <h2 style={{ margin: 0, fontSize: t.TEXT_TITLE, fontWeight: 600 }}>Import connections</h2>
        <p style={{ margin: 0, color: t.TEXT_MUTED, fontSize: t.TEXT_BODY }}>
          Merges a file written by <em>Export connections</em> into what you already have. A connection the file and
          this app both hold is updated in place; everything else is added, and nothing is removed.
        </p>
        <p style={{ margin: 0, color: t.TEXT_FAINT, fontSize: t.TEXT_BADGE }}>
          A connection whose password the file does not carry asks for one when you connect to it.
        </p>

        {error && <Callout>{error}</Callout>}
        {imported && <Callout tone="success">{summarise(imported)}</Callout>}

        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: t.GAP_SM, marginTop: t.GAP_XS }}>

...
export default function Titlebar({
  ...
  const assistantRunning = useAppSelector((s) =>
    Object.values(s.assistant.byTab).some(
      (conversation) => conversation.turnId !== null,
    ),
  );
  ...
  return (
    ...
    >
      ...
      {showingExport && (
        <ExportConnectionsDialog onClose={() => setShowingExport(false)} />
      )}
      {showingImport && (
        <ImportConnectionsDialog onClose={() => setShowingImport(false)} />
      )}
      {showingShortcuts && (
        <ShortcutsDialog onClose={() => setShowingShortcuts(false)} />
      )}
      <div
        style={{
          display: "flex",
      ...
      {needsTopResizeStrips && !maximized && (
        <WindowResizeTop onBegin={beginResize} />
      )}
    </header>
  );
}

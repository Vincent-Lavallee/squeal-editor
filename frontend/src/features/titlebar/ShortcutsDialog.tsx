            ...
            pushes the rows below it down the screen. */}
                ...
                <div style={scroller}>
                    {GROUPS.map((group) => (
                        <section
                            key={group}
                            style={{ display: 'flex', flexDirection: 'column', gap: t.GAP_XS }}
                        >
                            <Label>{group}</Label>
                            <ul style={list}>
                                {SHORTCUTS.filter((shortcut) => shortcut.group === group).map(
                                    (shortcut, i) => {
                                        ...
                                        return (
                                            ...
                                            >
                                                <span style={{ flex: 1, fontSize: t.TEXT_BODY }}>
                                                    {shortcut.label}
                                                </span>
                                                <span
                                                    style={{
                                                ...
                                                <Button
                                                    ...
                                                    onClick={() =>
                                                        listening ? stop() : start(shortcut.id)
                                                    }
                                                >
                                                    {listening
                                                        ? 'Press a key…'
                                                        : formatChord(bindings[shortcut.id]) || '—'}
                                                </Button>
                                            </li>
                                        );
                                    },

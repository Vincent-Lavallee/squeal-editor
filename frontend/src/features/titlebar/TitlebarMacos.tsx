...
const glyph = (colour: string, d: string) => (
  <svg width={DOT_SIZE} height={DOT_SIZE} viewBox={`0 0 ${DOT_SIZE} ${DOT_SIZE}`} aria-hidden="true"
    style={{ display: 'block' }}>
  ...
  return (
    <header style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: DOT_GAP, flex: 'none', paddingLeft: DOT_LEFT }}
        ...
        onMouseLeave={() => { setLightsHovered(false); setHovered(null); }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: LIGHTS_W, flex: 'none', height: '100%' }}>
        <button data-testid="titlebar-assistant" disabled={!onOpenAssistant}
          style={{
            display: 'flex', alignItems: 'center', justifyContent: 'center',
            width: ASSISTANT_W, height: '100%', border: 'none', padding: 0,
            position: 'relative',
          ...
          onMouseEnter={() => setAssistantHovered(true)} onMouseLeave={() => setAssistantHovered(false)}
          ...
          {assistantRunning && (
            <span data-testid="titlebar-assistant-busy" aria-hidden="true"
              style={{ position: 'absolute', top: 6, right: 4, width: 5, height: 5, borderRadius: t.RADIUS_PILL, background: t.ACCENT }} />
          )}
        </button>
      </div>
    </header>

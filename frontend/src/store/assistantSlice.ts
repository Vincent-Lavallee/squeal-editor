...
const conversationFor = (state: AssistantState, tabId: string): Conversation =>
...
async function runOneCall(
    ...
    if (!tool) {
        dispatch(
            toolAnswered({
                ...
                content: `No tool named ${toolCall.name}.`,
            }),
        );
        return;
    }
    ...
    try {
        ...
        dispatch(
            toolAnswered({
                ...
                outcome: 'failed',
                args: toolCall.arguments,
                content: 'Your arguments were not valid JSON. Send them again.',
            }),
        );
        return;
    }
    ...
    if (tool.mutating === true) {
        ...
        if (!skip) {
            const decision = await new Promise<{ approved: boolean; always: boolean }>(
                (resolve) => {
            ...
            if (!decision.approved) {
                answer(
                    'rejected',
                    'The user declined this. Do not retry it; ask them what to do instead.',
                );
                return;
            }
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(openConversation.pending, (state, action) => {
                state.byTab[action.meta.arg.tabId] = {
                    ...blankConversation(),

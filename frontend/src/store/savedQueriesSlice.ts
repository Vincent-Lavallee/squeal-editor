...
export const deleteSavedQuery = createAppThunk('savedQueries/delete', async (id: string, { rejectWithValue }) => {
  try {
...
const byName = (a: SavedQuery, b: SavedQuery): number => a.name.localeCompare(b.name, undefined, { sensitivity: 'base' });
...
export function useSavedQueries() {
  ...
  return {
    ...
    save: useCallback(
      (query: { id?: string; name: string; sql: string }) => dispatch(saveQuery(query)).unwrap(),
      [dispatch]
    ),

...
export const loadSettings = createAppThunk(
    'settings/load',
...
export function useBooleanSetting(
    ...
    const set = useCallback(
        (value: boolean) => void dispatch(saveSetting({ key, value: String(value) })),
        [dispatch, key],
    );
...
export function useShortcuts(): ShortcutSettings {
    ...
    const write = useCallback(
        (next: Partial<Bindings>) =>
            void dispatch(saveSetting({ key: KEYBINDINGS, value: JSON.stringify(next) })),
        [dispatch],
    );

    const rebind = useCallback(
        (id: ShortcutId, chord: string) => write({ ...overrides, [id]: chord }),
        [overrides, write],
    );

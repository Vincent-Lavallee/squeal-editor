  ...
  return {
    tabs: tabs.map((tab) => {
      if (tab.kind === 'grid') {
        ...
        return { kind: tab.kind, database: tab.database, table: tab.table, schema: tab.schema, title: tab.title, filter };
      }
      return { kind: tab.kind, database: tab.database, title: tab.title, sql: state.tabs.sqlByTab[tab.id] ?? '' };
    }),
    activeIndex: activeIndex < 0 ? null : activeIndex,
    nextQueryNo: state.tabs.nextQueryNo[connectionId] ?? tabs.length + 1,
    defaultDatabase: state.tabs.defaultDatabase[connectionId] ?? null,
  };
}

...
export const awsSsoLogin = createAppThunk(
    'awsSignIn/login',
    async (profile: string, { rejectWithValue }) => {
        try {
...
export function useAwsSignIn() {
    ...
    return {
        ...
        check: useCallback(
            (profile: string) => void dispatch(checkAwsCredentials(profile)),
            [dispatch],
        ),
        /** Resolves to whether the login actually completed, so a caller can act on it. */
        start: useCallback(
            async (profile: string) =>
                awsSsoLogin.fulfilled.match(await dispatch(awsSsoLogin(profile))),
            [dispatch],
        ),

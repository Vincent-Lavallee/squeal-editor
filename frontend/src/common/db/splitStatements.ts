...
function statementSpans(sql: string, dialect: SqlDialect): StatementSpan[] {
  ...
  while (i < sql.length) {
    ...
    if (ch === "'" || ch === '"' || ch === '`') {
      ...
      const escapes = ch !== '`' && (lexis.backslashEscapes || (lexis.escapeStrings && isEscapeStringOpener(sql, i)));
    ...
    if (lexis.delimiterDirective && (ch === 'd' || ch === 'D') && !significant && startsLine(sql, i)) {
...
const LEXIS: Record<SqlDialect, Lexis> = {
  mysql: { hashComments: true, dashCommentNeedsSpace: true, nestedBlockComments: false, backslashEscapes: true, escapeStrings: false, dollarQuotes: false, delimiterDirective: true },
  pgsql: { hashComments: false, dashCommentNeedsSpace: false, nestedBlockComments: true, backslashEscapes: false, escapeStrings: true, dollarQuotes: true, delimiterDirective: false },
  sql: { hashComments: false, dashCommentNeedsSpace: false, nestedBlockComments: false, backslashEscapes: false, escapeStrings: false, dollarQuotes: false, delimiterDirective: false },
};
...
function unquoteDelimiter(token: string): string {
  const quote = token[0];
  const quoted = (quote === '"' || quote === "'" || quote === '`') && token.length > 1 && token.endsWith(quote);
...
function pastBlockComment(sql: string, open: number, nested: boolean): number {
  ...
  let i = open + 2;
  while (i < sql.length) {
    if (nested && sql[i] === '/' && sql[i + 1] === '*') { depth += 1; i += 2; continue; }
    if (sql[i] === '*' && sql[i + 1] === '/') {
      depth -= 1;
      i += 2;
      if (depth === 0) return i;

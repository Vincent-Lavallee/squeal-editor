...
describe('the password on disk', () => {
    ...
    test('is a real AES-GCM envelope, and tampering with it is detected', async () => {
        ...
        const original = Buffer.from(
            (
                db
                    .query('SELECT password FROM saved_connections WHERE name = ?')
                    .get('with-password') as {
                    password: Uint8Array;
                }
            ).password,
        );
        ...
        db.run('UPDATE saved_connections SET password = ? WHERE name = ?', [
            tampered,
            'with-password',
        ]);
...
describe('IAM authentication', () => {
    const IAM_CONFIG = {
        ...PG_SERVER,
        ssl: true,
        iam: { profile: 'squeal-test', region: 'us-east-1' },
    };
...
describe('across a restart', () => {
    test('connections and their passwords survive a new extension process', async () => {
        ...
        const res = (await h.ok('db.saved.connect', { id: survivor.id })) as {
            connectionId: string;
        };
        expect(res.connectionId).toBeTruthy();
        await h.ok('db.disconnect', { connectionId: res.connectionId });
...
describe('starred tables', () => {
    const stars = async (
        ...
        (
            (await h.ok('db.stars.list', { savedConnectionId })) as {
                stars: { database: string; schema?: string; table: string }[];
            }
        ).stars;
    ...
    test('starring adds it, unstarring removes it, and both are idempotent', async () => {
        ...
        expect(await stars(saved.id)).toEqual([
            { database: FIXTURE_DB, schema: 'public', table: 'users' },
        ]);
    ...
    test('a table with no schema stars the same as one twice, not once', async () => {
        ...
        expect(await stars(saved.id)).toEqual([
            { database: FIXTURE_DB, schema: undefined, table: 'orders' },
        ]);
    ...
    test('deleting a connection takes its stars with it', async () => {
        ...
        const db = new Database(DB_FILE);
        const remaining = db
            .query('SELECT COUNT(*) AS n FROM stars WHERE connection_id = ?')
...
describe('saved queries', () => {
    ...
    test('saving with an id replaces that query rather than adding a second', async () => {
        ...
        expect((await queries()).filter((q) => q.name === 'reused')).toEqual([
            { id: saved.id, name: 'reused', sql: 'SELECT 2' },
        ]);
    ...
    test('an id that no longer names a query is refused, not re-created under it', async () => {
        ...
        const res = await h.dispatch('queries.save', {
            id: saved.id,
            name: 'gone',
            sql: 'SELECT 2',
        });
...
describe('assistant conversations', () => {
    ...
    const body = (text: string): string =>
        JSON.stringify({ messages: [{ role: 'user', content: text }], tools: {} });

    const listing = async (): Promise<AiConversationSummary[]> =>
        ((await h.ok('conversations.list', {})) as { conversations: AiConversationSummary[] })
            .conversations;

    const fetch = async (id: string): Promise<AiConversation | null> =>
        ((await h.ok('conversations.get', { id })) as { conversation: AiConversation | null })
            .conversation;

    test('one is written, listed, read back whole and deleted', async () => {
        await h.ok('conversations.save', {
            id: 'c-round-trip',
            title: 'Slow orders',
            body: body('why is this slow'),
        });
    ...
    test('they are listed newest first, by when each was last written', async () => {
        ...
        await h.ok('conversations.save', {
            id: 'c-older',
            title: 'Older, returned to',
            body: body('a c'),
        });

        const mine = (await listing())
            .map((c) => c.id)
            .filter((id) => id.startsWith('c-old') || id.startsWith('c-new'));
...
describe('workspaces', () => {
    ...
    test('a connection name is a label: it need not be unique anywhere', async () => {
        ...
        const inA = await save({
            workspaceId: a.id,
            name: 'api',
            config: PG_SERVER,
            password: { mode: 'none' },
        });
        const inB = await save({
            workspaceId: b.id,
            name: 'api',
            config: PG_SERVER,
            password: { mode: 'none' },
...
describe('exporting and importing', () => {
    ...
    const documentIn = (path: string): Document =>
    ...
    test('importing puts a deleted connection back, and it still reaches the server', async () => {
        ...
        const res = (await h.ok('db.saved.connect', { id: carried.id })) as {
            connectionId: string;
        };
        await h.ok('db.disconnect', { connectionId: res.connectionId });
    });
    ...
    test('a file written by a newer format says so rather than being half-read', async () => {
        writeFileSync(
            NONSENSE,
            JSON.stringify({ squealConnections: 99, workspaces: [], connections: [] }),
        );
        const res = await h.dispatch('db.saved.import', { path: NONSENSE });
        expect(res.ok).toBe(false);
    ...
    test('a connection naming a workspace the file does not describe lands nothing at all', async () => {
        ...
        writeFileSync(
            ...
            JSON.stringify({
                ...doc,
                workspaces: [],
                connections: doc.connections.map((c) => ({ ...c, id: Bun.randomUUIDv7() })),
            }),
        );
    ...
    test('a missing file is a failed import, not a crash', async () => {
        const res = await h.dispatch('db.saved.import', {
            path: join(DATA_DIR, 'no-such-file.json'),
        });
        expect(res.ok).toBe(false);
...
const stamps = (): Stamp[] => {
    const db = new Database(DB_FILE);
    const rows = db
        .query('SELECT version, name, origin FROM schema_migrations ORDER BY version')
        .all() as Stamp[];
    db.close();
    return rows;
};
...
const UNDO: Record<string, string[]> = {
    ...
    'drop-workspace-colour': [
        "ALTER TABLE workspaces ADD COLUMN color TEXT NOT NULL DEFAULT 'slate'",
    ],
...
function rewindTo(
    name: string,
    { forgetVersion = false } = {},
): { connections: string[]; workspaces: string[] } {
    ...
    for (const migration of [...MIGRATIONS].reverse()) {
        ...
        if (!undo)
            throw new Error(
                `No undo for "${migration.name}", which a rewind to "${name}" has to pass.`,
            );
...
describe('migrating a store written before workspaces', () => {
  ...
  `;
    ...
    test('every connection lands in a default workspace, as Local, still usable', async () => {
        ...
        const legacyNames = (
            db.query('SELECT name FROM saved_connections').all() as { name: string }[]
        ).map((r) => r.name);
        db.close();
        ...
        expect(rows.filter((r) => r.version > first).every((r) => r.origin === 'applied')).toBe(
            true,
        );
...
describe('migrating a store written before SSL', () => {
    test('every connection stays plaintext, and still connects', async () => {
        ...
        const res = (await h.ok('db.saved.connect', { id: migrated.id })) as {
            connectionId: string;
        };
        expect(res.connectionId).toBeTruthy();
        await h.ok('db.disconnect', { connectionId: res.connectionId });
...
describe('migrating a store written before the workspace colour column was dropped', () => {
    test('every workspace loses its stored colour, and stays usable', async () => {
        ...
        const shape = (
            after.query('PRAGMA table_info(workspaces)').all() as { name: string }[]
        ).map((c) => c.name);
        after.close();
...
describe('migrating a store written before connection names could repeat', () => {
    test('the rebuild keeps the stars and sessions hanging off it, and names may then repeat', async () => {
        ...
        const kept = db
            ...
            .get(conn.id) as { snapshot: string } | null;
        db.close();
    ...
    test('a colour column that is nullable on disk still rebuilds', async () => {
        ...
        const nulls = (
            db.query('SELECT COUNT(*) AS n FROM saved_connections WHERE color IS NULL').get() as {
                n: number;
            }
        ).n;
        db.close();
...
describe('adopting a store written before there were versions', () => {
    test('it is placed by its shape, then walked to the top', async () => {
        ...
        expect(rows.map((r) => r.version)).toEqual(ALL_VERSIONS);
        expect(rows.filter((r) => r.version <= readOnly).every((r) => r.origin === 'adopted')).toBe(
            true,
        );
        expect(rows.filter((r) => r.version > readOnly).every((r) => r.origin === 'applied')).toBe(
            true,
        );

...
import { awsFailureKind, credentialStatus, mapAwsError, readPrompts } from '../extensions/db/iam.ts';
...
describe('awsFailureKind', () => {
  test('the SDK\'s own name for a lapsed token is enough', () => {
    expect(awsFailureKind(Object.assign(new Error('anything'), { name: 'TokenProviderError' }))).toBe('expired-sso');
  });

  test('the text is the fallback, since the name is not stable across the chain', () => {
    expect(awsFailureKind(new Error('The SSO session associated with this profile has expired.'))).toBe('expired-sso');
  });

  test('a missing profile is its own kind, because signing in would not fix it', () => {
    expect(awsFailureKind(new Error('Profile x could not be found in shared config files.'))).toBe('missing-profile');
    ...
    // should.
    expect(
      awsFailureKind(new Error('Could not resolve credentials using profile: [x] in configuration/credentials file(s).'))
  ...
  test('an expired session is claimed before the missing-profile phrasings', () => {
    ...
    const err = Object.assign(new Error('Could not resolve credentials using profile: [x]. SSO session has expired.'), {
      name: 'TokenProviderError',
    });
...
describe('readPrompts', () => {
  ...
  test('a chunk boundary anywhere gives the same answer', async () => {
    ...
    for (const size of [1, 3, 17, 64]) {
      ...
      expect(seen.at(-1)).toEqual({ url: 'https://device.sso.us-east-1.amazonaws.com/', code: 'MRVK-QPZD' });
    }
  });
  ...
  test('output with no URL reports nothing at all', async () => {
    const seen: AwsSsoPrompt[] = [];
    const collected = await readPrompts(chunked('aws: [ERROR]: The config profile (x) could not be found\n', 9), (p) => seen.push({ ...p }));
    expect(seen).toEqual([]);

...
describe.each([
    ...
    beforeAll(async () => {
        const res = (await h.ok('db.connect', { config })) as {
            connectionId: string;
            databases: string[];
        };
    ...
    const query = async (
        sql: string,
        database: string | undefined = fixtureDb,
    ): Promise<QueryResult> =>
    ...
    const listTables = async (): Promise<TableInfo[]> =>
        (
            (await h.ok('db.tables', { connectionId, database: fixtureDb })) as {
                tables: TableInfo[];
            }
        ).tables;
    ...
    const searchTables = async (
        search?: string,
        limit?: number,
    ): Promise<{ tables: TableInfo[]; truncated: boolean }> =>
    ...
    const columnsOf = async (table: string, schema?: string): Promise<ColumnInfo[]> =>
        (
            (await h.ok('db.columns', { connectionId, database: fixtureDb, table, schema })) as {
                columns: ColumnInfo[];
            }
        ).columns;
    ...
    test("lists a table's columns in the order the table declares them", async () => {
        ...
        expect(columns.map((c) => c.name).slice(0, 4)).toEqual([
            'id',
            'name',
            'email',
            'created_at',
    ...
    test.if(expectSchemaQualified)(
        ...
        async () => {
            ...
            expect((await columnsOf('reporting.daily_stats')).map((c) => c.name)).toEqual([
                'day',
                'hits',
            ]);
        },
    ...
    const relationships = async (): Promise<DiagramTable[]> =>
        (
            (await h.ok('db.relationships', { connectionId, database: fixtureDb })) as {
                tables: DiagramTable[];
            }
        ).tables;
    ...
    test('a composite foreign key survives whole, paired by key position', async () => {
        ...
        const columns = (await h.ok('db.columns', {
            connectionId,
            database: fixtureDb,
            table: 'cities',
        })) as {
    ...
    test('a builder condition narrows the page', async () => {
        const page = await filtered(
            'users',
            where([{ column: 'name', operator: '=', value: 'Ada' }]),
        );
        expect(page.result.rows).toHaveLength(1);
    ...
    test('IS NULL and IS NOT NULL compare against no value', async () => {
        ...
        const nulls = await filtered(
            'users',
            where([{ column: 'email', operator: 'IS NULL', value: '' }]),
        );
        expect(nulls.result.rows).toHaveLength(1);
        expect(nulls.result.rows[0]![1]).toBe('Grace');

        const notNulls = await filtered(
            'users',
            where([{ column: 'email', operator: 'IS NOT NULL', value: '' }]),
        );
    ...
    test('IN binds one placeholder per item', async () => {
        const page = await filtered(
            'tags',
            where([{ column: 'label', operator: 'IN', value: 'red, blue' }]),
        );
        expect(page.result.rows).toHaveLength(2);

        const one = await filtered(
            'tags',
            where([{ column: 'label', operator: 'IN', value: 'red' }]),
        );
    ...
    test('an empty builder is no filter at all', async () => {
        ...
        expect(
            (await filtered('users', where([{ column: '', operator: '=', value: 'x' }]))).result
                .rows,
        ).toHaveLength(2);
    });
    ...
    test('an empty raw clause is no filter', async () => {
        expect((await filtered('users', { kind: 'raw', where: '   ' })).result.rows).toHaveLength(
            2,
        );
    });
    ...
    test('an operator outside the set is refused, not authored', async () => {
        ...
        const res = await h.dispatch('db.browse', {
            ...
            filter: {
                kind: 'builder',
                conjunction: 'AND',
                conditions: [{ column: 'name', operator: 'DROP', value: 'x' }],
            },
        });
        expect(res.ok).toBe(false);
    ...
    test('a page comes back in the order it was asked for, both ways', async () => {
        expect(labelsOf(await sorted('tags', { column: 'label', direction: 'asc' }))).toEqual([
            'blue',
            'red',
        ]);
        expect(labelsOf(await sorted('tags', { column: 'label', direction: 'desc' }))).toEqual([
            'red',
            'blue',
        ]);
    });
    ...
    test('a sorted query is wrapped, and comes back in that order', async () => {
        const asc = await sortedQuery('SELECT label, weight FROM tags', {
            column: 'label',
            direction: 'asc',
        });
        expect(asc.rows.map((r) => String(r[0]))).toEqual(['blue', 'red']);

        const desc = await sortedQuery('SELECT label, weight FROM tags', {
            column: 'label',
            direction: 'desc',
        });
    ...
    test('sorting a query changes the order and never the rows', async () => {
        ...
        const sortedRows = await sortedQuery('SELECT label FROM tags', {
            column: 'label',
            direction: 'desc',
        });
        expect(sortedRows.rows).toHaveLength(plain.rows.length);
        expect(new Set(sortedRows.rows.map((r) => String(r[0])))).toEqual(
            new Set(plain.rows.map((r) => String(r[0]))),
        );
    });
    ...
    test("a sort overrides the statement's own ORDER BY rather than fighting it", async () => {
        ...
        // append to at all.
        const res = await sortedQuery('SELECT label FROM tags ORDER BY label ASC', {
            column: 'label',
            direction: 'desc',
        });
    ...
    test('a trailing semicolon does not break the wrap', async () => {
        ...
        const res = await sortedQuery('SELECT label FROM tags;  ', {
            column: 'label',
            direction: 'asc',
        });
    ...
    const ddlOf = async (
        ...
        (
            (await h.ok('db.ddl', { connectionId, database: fixtureDb, table, schema, kind })) as {
                ddl: string;
            }
        ).ddl;
    ...
    const listFunctions = async (): Promise<FunctionInfo[]> =>
        (
            (await h.ok('db.functions', { connectionId, database: fixtureDb })) as {
    ...
    const functionDdlOf = async (func: FunctionInfo): Promise<string> =>
        (
            (await h.ok('db.functionDdl', { connectionId, database: fixtureDb, func })) as {
                ddl: string;
            }
        ).ddl;
    ...
    test.if(label !== 'sqlite')(
        'renders a function definition, and a procedure through the other verb',
        async () => {
            const functions = await listFunctions();
    ...
    test.if(expectSchemaQualified)(
        ...
        async () => {
            ...
            expect(
                remaining.some((t) => t.name === 'daily_stats' && t.schema === 'reporting'),
            ).toBe(true);
        },
    );
    ...
    test('drops a table, and it is gone afterwards', async () => {
        ...
        const { tables } = (await h.ok('db.tables', { connectionId, database: fixtureDb })) as {
            tables: TableInfo[];
        };
    ...
    test('a bad statement errors without killing the connection', async () => {
        const bad = await h.dispatch('db.query', {
            connectionId,
            database: fixtureDb,
    ...
    test('disconnect closes the connection for good', async () => {
        ...
        const res = await h.dispatch('db.query', {
            connectionId: temp,
            database: fixtureDb,
            sql: 'SELECT 1',
        });
    ...
    test('holds several connections at once, each its own', async () => {
        ...
        expect(
            (await h.dispatch('db.query', { connectionId: a.connectionId, sql: 'SELECT 1' })).ok,
        ).toBe(false);
    ...
    describe('editable grid', () => {
        ...
        test('db.tableKey answers the same identity without paging or writing', async () => {
            ...
            const tableKey = async (table: string): Promise<string[] | null> =>
                (
                    (await h.ok('db.tableKey', { connectionId, database: fixtureDb, table })) as {
        ...
        test('writes edits and deletes back as one batch', async () => {
            ...
            await query(
                "INSERT INTO zz_edit_test (id, name, n) VALUES (1, 'a', 10), (2, 'b', 20), (3, 'c', 30)",
            );
...
describe.each([
    [
        ...
        (id: string) => `SELECT pg_terminate_backend(${id})`,
    ],
    ...
    test('the extension survives it, says so, and reconnects on the next query', async () => {
        ...
        const lost = h.waitFor(
            ...
            (d: { connectionId: string; state: string }) =>
                d.connectionId === victim && d.state === 'lost',
        );
        ...
        const untouched = (await h.ok('db.query', {
            connectionId: killer,
            sql: 'SELECT 1 AS ok',
        })) as QueryResult;
        ...
        const restored = h.waitFor(
            'connection.state',
            (d: { connectionId: string; state: string }) =>
                d.connectionId === victim && d.state === 'restored',
        );
    ...
    test('a drop landing on a running query does not poison the connection', async () => {
        ...
        const failed = await h.dispatch('db.query', {
            connectionId: victim,
            sql: killSql(await backendId()),
        });
        expect(failed.ok).toBe(false);

        const recovered = (await h.ok('db.query', {
            connectionId: victim,
            sql: 'SELECT 1 AS ok',
        })) as QueryResult;
    ...
    test('disconnecting a dropped connection returns promptly', async () => {
        const doomed = ((await h.ok('db.connect', { config })) as { connectionId: string })
            .connectionId;
        const id = ((await h.ok('db.query', { connectionId: doomed, sql: whoAmI })) as QueryResult)
            .rows[0]![0];

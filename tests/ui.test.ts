...
const clickRail = (i: number) =>
    `document.querySelectorAll('[data-testid="rail-item"]')[${i}].click(); true;`;
...
const clickTab = (label: string) =>
    `${tab(label)}.querySelector('[data-testid="tab-pick"]').click(); true;`;
const closeTab = (label: string) =>
    `${tab(label)}.querySelector('[data-testid="tab-close"]').click(); true;`;
...
const setEditorText = (sql: string) =>
    `window.squealEditor.setValue(${JSON.stringify(sql)}); true;`;
/**
...
async function acceptSuggestion(sql: string): Promise<string> {
    await suggest(sql);
    await app.evaluate(
        `window.squealEditor.trigger('test', 'acceptSelectedSuggestion', {}); true;`,
    );
    await Bun.sleep(300);
 ...
 * shape real rows do, so counting them without excluding `.skeleton` makes a
 * `waitFor` on this resolve on the loading state instead of the data it's
...
const dblClick = (expr: string) =>
    `${expr}.dispatchEvent(new MouseEvent('dblclick', { bubbles: true })); true;`;
...
async function disconnect(): Promise<void> {
    await app.evaluate(
        `document.querySelector('[data-testid="statusbar-disconnect"]').click(); true;`,
    );
    await Bun.sleep(800);
}
...
describe.skipIf(!UI_ENABLED)('the real app', () => {
    beforeAll(async () => {
        app = await launchApp({
            SQUEAL_DATA_DIR: DATA_DIR,
            SQUEAL_KEYCHAIN_SERVICE: KEYCHAIN_SERVICE,
        });
    });
    ...
    afterAll(async () => {
        ...
        await Bun.secrets
            .delete({ service: KEYCHAIN_SERVICE, name: 'connection-key' })
            .catch(() => undefined);
    ...
    describe('postgres', () => {
        ...
        test('connects and renders the shell', async () => {
            const shell = await app.evaluate<boolean>(
                `!!document.querySelector('[data-testid="sidebar"]')`,
            );
            if (!shell) {
                throw new Error(
                    await app.evaluate<string>(
                        `document.querySelector('[data-testid="callout"]')?.textContent ?? 'no error shown'`,
                    ),
                );
            }
        ...
        test('offers the databases in the picker', async () => {
            const dbs = await app.evaluate<string[]>(
                `${REACT_SETTERS} optionsOf('sidebar-db-select', 'label');`,
            );
        ...
        (async () => {
          ...
          return {
            ...
            await app.waitFor(
                `document.querySelector('[data-testid="sidebar-tables-refresh"] .spin') ? null : true`,
            );
        ...
        test('a schema heading opens and closes, taking only its own relations with it', async () => {
            ...
            await Bun.sleep(300);
            expect(await app.evaluate<string[]>(treeLabelsIn('reporting'))).toContain(
                'daily_stats',
            );
        ...
        test('a table expands in place to show its columns, marking the primary key', async () => {
            ...
            const hasKey = (
                name: string,
            ) => `!![...${item}.querySelectorAll('[data-testid="tree-col"]')]
        ...
        .querySelector('[data-testid="tree-key"]')`;
            ...
            expect(
                await app.evaluate<number>(
                    `${item}.querySelectorAll('[data-testid="tree-col"]').length`,
                ),
            ).toBe(0);
        });
        ...
        test('clicking a table opens a grid tab, leaving the editor tab alone', async () => {
            ...
            expect(
                await app.evaluate<number>(`document.querySelectorAll('.grid tbody tr').length`),
            ).toBe(2);

            // A grid tab spends none of the screen on an editor nobody asked for.
            expect(
                await app.evaluate<boolean>(
                    `!!document.querySelector('[data-testid="main-grid"]')`,
                ),
            ).toBe(true);
            ...
            await Bun.sleep(400);
            expect(await app.evaluate<string | null>(editorText)).toBe(
                'SELECT 1 -- still being written',
            );
        });
        ...
        test('closing the last tab leaves an empty state', async () => {
            ...
            expect(await app.evaluate<string[]>(tabLabels)).toEqual([]);
            const note = await app.evaluate<string>(
                `document.querySelector('[data-testid="note-muted"]')?.textContent ?? ''`,
            );
        ...
        test('the tree is usable from the empty state, with no tab open at all', async () => {
            expect(await app.evaluate<string[]>(tabLabels)).toEqual([]);
            expect(
                await app.evaluate<string | null>(
                    `document.querySelector('[data-testid="sidebar-db-select"]').getAttribute('aria-disabled')`,
                ),
            ).toBeNull();
        ...
        test('a new tab from the empty state lands on a database', async () => {
            ...
            const picked = await app.evaluate<string>(
                `${REACT_SETTERS} selectValue('sidebar-db-select');`,
            );
            expect(picked).not.toBe('');
            const options = await app.evaluate<string[]>(
                `${REACT_SETTERS} optionsOf('sidebar-db-select', 'value');`,
            );
        ...
        test('a table that fits on one page has no pager', async () => {
            ...
            expect(
                await app.evaluate<number>(
                    `document.querySelectorAll('[data-testid="results-pager"]').length`,
                ),
            ).toBe(0);
            await app.evaluate(closeTab('users'));
            await Bun.sleep(300);
        ...
        test('a table larger than a page pages forward and back', async () => {
            ...
            expect(
                await app.evaluate<string>(
                    `document.querySelector('.grid tbody td.gutter').textContent`,
                ),
            ).toBe('101');
        ...
        test('a hand-typed query sorts too, wrapped rather than rewritten', async () => {
            ...
            const names = `[...document.querySelectorAll('.grid tbody tr')].map(r => r.querySelectorAll('td:not(.gutter)')[1].textContent)`;
            await app.waitFor(
                `JSON.stringify(${names}) === JSON.stringify(['Ada', 'Grace']) ? true : null`,
            );
            expect(await app.evaluate<string[]>(names)).toEqual(['Ada', 'Grace']);
        ...
        test('a filter narrows the grid, and clearing it restores the table', async () => {
            ...
            expect(
                await app.evaluate<number>(
                    `document.querySelectorAll('[data-testid="filter-condition"]').length`,
                ),
            ).toBe(1);
            ...
            expect(
                await app.evaluate<boolean>(
                    `document.querySelector('[data-testid="filter-apply"]').disabled`,
                ),
            ).toBe(false);
        ...
        true;`);
            await Bun.sleep(300);
            await app.evaluate(
                `document.querySelector('[data-testid="filter-toggle-form"]').click(); true;`,
            );
            await Bun.sleep(400);
            ...
            expect(
                await app.evaluate<string>(
                    `document.querySelector('[data-testid="filter-raw"]').value`,
                ),
            ).toBe(`"name" = 'O''Hara'`);

            // And it is text that actually runs, not a label.
            await app.evaluate(
                `${REACT_SETTERS} setNative(document.querySelector('[data-testid="filter-raw"]'), "name = 'Ada'"); true;`,
            );
            await Bun.sleep(300);
            await app.evaluate(
                `document.querySelector('[data-testid="filter-apply"]').click(); true;`,
        ...
        true;`);
            ...
            ).toBe(`"eventType" = 'page_view'`);

            await app.evaluate(
                `document.querySelector('[data-testid="filter-apply"]').click(); true;`,
            );
            await app.waitFor(
                `(${rowCount}) === 1 || document.querySelector('[data-testid="note-error"]') ? true : null`,
            );
        ...
        true;`);
            await Bun.sleep(300);
            await app.evaluate(
                `document.querySelector('[data-testid="filter-add"]').click(); true;`,
            );
            await Bun.sleep(300);
            ...
            expect(
                await app.evaluate<string>(
                    `document.querySelectorAll('[data-testid="filter-column"]')[0].getAttribute('data-value')`,
                ),
            ).toBe('name');
            expect(
                await app.evaluate<string>(
                    `document.querySelectorAll('[data-testid="filter-value"]')[0].value`,
                ),
            ).toBe('Ada');
            ...
            await app.evaluate(
                `document.querySelectorAll('[data-testid="filter-remove"]')[1].click(); true;`,
            );
            await Bun.sleep(300);
            await app.evaluate(
                `document.querySelector('[data-testid="filter-apply"]').click(); true;`,
        ...
        true;`);
            ...
            await app.waitFor(`document.querySelector('[data-testid="note-error"]') ? true : null`);
            ...
            expect(
                await app.evaluate<number>(
                    `document.querySelectorAll('[data-testid="filter-raw"]').length`,
                ),
            ).toBe(1);
            expect(
                await app.evaluate<string>(
            ...
            await Bun.sleep(300);
            const columnOptions = await app.evaluate<string[]>(
                `${REACT_SETTERS} optionsOf('filter-column', 'value');`,
            );
        ...
        test('NULL is rendered distinctly, not as empty or "null"', async () => {
            ...
            expect(
                await app.evaluate<boolean>(
                    `!!document.querySelector('[data-testid="null-value"]')`,
                ),
            ).toBe(true);
        });
        ...
        test('Ctrl+Enter runs the editor contents', async () => {
            ...
            const headers = `[...document.querySelectorAll('[data-testid="grid-col-name"]')].map(e => e.textContent)`;
            await app.waitFor(
                `JSON.stringify(${headers}) === JSON.stringify(['name', 'email']) ? true : null`,
            );

            expect(await app.evaluate<string[]>(headers)).toEqual(['name', 'email']);
        ...
        test('a selection is what runs, and the button says which', async () => {
            ...
            expect(
                await app.evaluate<string>(
                    `document.querySelector('[data-testid="run-btn"]').textContent`,
                ),
            ).toBe('Run selection');
        ...
        test('a selection of nothing but whitespace runs nothing, rather than everything', async () => {
            await app.evaluate(
                setEditorText('SELECT name FROM users;\n   \nSELECT email FROM users;'),
            );
            await Bun.sleep(200);
        ...
        test('a run of several statements gets a numbered tab each', async () => {
            ...
            await app.evaluate(
                `document.querySelectorAll('[data-testid="statement-tab"]')[1].click(); true;`,
            );
            await Bun.sleep(300);
      ...
      return true;
        ...
        const runStatement = `document.dispatchEvent(new KeyboardEvent('keydown', { key: 'Enter', ctrlKey: true, shiftKey: true, bubbles: true })); true;`;
        ...
        test('a failing statement stops the batch and the strip says what did not run', async () => {
            ...
            expect(
                await app.evaluate<string>(
                    `document.querySelector('[data-testid="statements-not-run"]')?.textContent ?? ''`,
                ),
            ).toBe('1 not run');
            ...
            await app.evaluate(
                `document.querySelectorAll('[data-testid="statement-tab"]')[0].click(); true;`,
            );
            await Bun.sleep(300);
            expect(await app.evaluate<string[]>(gridHeaders)).toEqual(['name']);
        });
        ...
        test('a keyless table is read-only and says why', async () => {
            ...
            await app.waitFor(`document.querySelector('[data-testid="results-ro"]') ? true : null`);

            expect(
                await app.evaluate<string>(
                    `document.querySelector('[data-testid="results-ro"]')?.textContent ?? ''`,
                ),
            ).toMatch(/no primary or unique key/i);
            ...
            expect(
                await app.evaluate<number>(
                    `document.querySelectorAll('[data-testid="cell-edit-input"]').length`,
                ),
            ).toBe(0);
        ...
        test('an editable table stages an edit and saves it', async () => {
            ...
            expect(
                await app.evaluate<number>(
                    `document.querySelectorAll('[data-testid="results-ro"]').length`,
                ),
            ).toBe(0);
        ...
        true;`);
            ...
            expect(
                await app.evaluate<string>(
                    `document.querySelector('[data-testid="results-savebar"] span').textContent`,
                ),
            ...
            const weights = `[...document.querySelectorAll('.grid tbody tr')].map(tr => tr.querySelectorAll('td:not(.gutter)')[1].textContent)`;
            await app.waitFor(
                `!document.querySelector('[data-testid="results-savebar"]') && (${weights}).includes(${JSON.stringify(next)}) ? true : null`,
            );
            ...
            expect(
                await app.evaluate<number>(
                    `document.querySelectorAll('[data-testid="results-savebar"]').length`,
                ),
            ).toBe(0);
            expect(await app.evaluate<string[]>(weights)).toContain(next);
        ...
        test('Set NULL and Delete stage on the grid, and Discard clears them', async () => {
            ...
            await app.evaluate(dblClick(gridCell(0, 1)));
            await app.waitFor(
                `document.querySelector('[data-testid="cell-edit-null"]') ? true : null`,
            );
            await app.evaluate(
            ...
            expect(
                await app.evaluate<boolean>(
                    `!!${gridCell(0, 1)}.querySelector('[data-testid="null-value"]')`,
                ),
            ).toBe(true);

            // Stage a delete on the second row: select its gutter, press Delete.
            await app.evaluate(
                `document.querySelectorAll('.grid tbody tr')[1].querySelector('.gutter').click(); true;`,
            );
            await Bun.sleep(150);
            await app.evaluate(
            ...
            expect(
                await app.evaluate<number>(
                    `document.querySelectorAll('.grid__row--deleted').length`,
                ),
            ).toBe(1);
            ...
            expect(
                await app.evaluate<number>(
                    `document.querySelectorAll('[data-testid="null-value"]').length`,
                ),
            ).toBe(0);
        ...
        test('clicking a cell selects it alone, arrow keys move it, and Ctrl+C copies its value', async () => {
            ...
            expect(
                await app.evaluate<boolean>(
                    `${gridCell(0, 0)}.classList.contains('grid__cell--selected')`,
                ),
            ).toBe(true);
            ...
            expect(
                await app.evaluate<boolean>(
                    `${gridCell(0, 1)}.classList.contains('grid__cell--selected')`,
                ),
            ).toBe(true);
            expect(
                await app.evaluate<boolean>(
        ...
        test('highlights with the dialect the engine reported', async () => {
            expect(
                await app.evaluate<string>(`window.squealEditor.getModel().getLanguageId()`),
            ).toBe('pgsql');
        });
        ...
        test('find and replace is available over the editor text', async () => {
            ...
            await Bun.sleep(200);
            await app.evaluate(
                `window.squealEditor.getAction('editor.action.startFindReplaceAction').run()`,
            );
            await Bun.sleep(400);
            expect(
                await app.evaluate<boolean>(`!!document.querySelector('.find-widget.visible')`),
            ).toBe(true);
        ...
        c.replaceAll(); true;`);
            ...
            await app.evaluate(
                `window.squealEditor.trigger('test', 'closeFindWidget', null); true;`,
            );
        });
        ...
        test('a schema-qualified relation completes on its columns', async () => {
            ...
            expect(
                await suggest('SELECT reporting.daily_stats.| FROM reporting.daily_stats'),
            ).toContain('hits');
        });
        ...
        test('a mixed-case column is inserted quoted, and the query it makes runs', async () => {
            expect(await acceptSuggestion('SELECT eventT| FROM users')).toBe(
                'SELECT "eventType" FROM users',
            );
        ...
        test('a column that needs no quoting is inserted bare', async () => {
            ...
            // anyone writes.
            expect(await acceptSuggestion('SELECT ema| FROM users')).toBe(
                'SELECT email FROM users',
            );
        });
        ...
        test('a grid tab moves database from the caret on Search', async () => {
            ...
            expect(
                await app.evaluate<string>(
                    `document.querySelector('[data-testid="results-db"]').textContent`,
                ),
            ).toBe('shop');
        ...
        test('right-clicking a table opens its action menu', async () => {
            ...
            expect(
                await app.evaluate<number>(
                    `document.querySelectorAll('[data-testid="context-menu"]').length`,
                ),
            ).toBe(0);
        });
        ...
        test('starring a table pins it above the rest, and unstarring returns it', async () => {
            ...
            expect(
                await app.evaluate<boolean>(
                    `!!document.querySelector('[data-testid="tree-pinned"]')`,
                ),
            ).toBe(true);
        ...
        test('"Open definition" opens the CREATE in a new named editor tab', async () => {
            const before = await app.evaluate<number>(
            ...
            await app.waitFor(
                `(${editorText}) && (${editorText}).includes('CREATE TABLE') ? true : null`,
            );
            ...
            // clean strip.
            await app.evaluate(
                `document.querySelector('[data-testid="tab-pick"][aria-selected="true"]').parentElement.querySelector('[data-testid="tab-close"]').click(); true;`,
            );
            await Bun.sleep(300);
        });
        ...
        test('dropping asks for the name typed back, and cancels cleanly', async () => {
            ...
            expect(
                await app.evaluate<boolean>(`!!document.querySelector('[data-testid="modal"]')`),
            ).toBe(true);
            ...
            await app.evaluate(
                `${REACT_SETTERS} setNative(document.querySelector('[data-testid="modal-input"]'), 'not-the-name'); true;`,
            );
            await Bun.sleep(150);
            expect(await app.evaluate<boolean>(confirmDisabled)).toBe(true);

            await app.evaluate(
                `${REACT_SETTERS} setNative(document.querySelector('[data-testid="modal-input"]'), 'users'); true;`,
            );
            await Bun.sleep(150);
            ...
            await app.evaluate(
                `[...document.querySelectorAll('[data-testid="modal"] button')].find(e => e.textContent === 'Cancel').click(); true;`,
            );
            await Bun.sleep(300);
            expect(
                await app.evaluate<boolean>(`!!document.querySelector('[data-testid="modal"]')`),
        ...
        test('a read-only connection disables Drop', async () => {
            const env = await app.evaluate<string>(
                `document.querySelector('[data-testid="statusbar-env"]').textContent`,
            );

            // Lock it -- turning read-only on is the safe direction, so it is immediate.
            await app.evaluate(
                `document.querySelector('[data-testid="statusbar-lock"]').click(); true;`,
            );
            await Bun.sleep(500);
            ...
            await Bun.sleep(300);
            await app.evaluate(
                `${REACT_SETTERS} setNative(document.querySelector('[data-testid="modal-input"]'), ${JSON.stringify(env)}); true;`,
            );
            await Bun.sleep(150);
            await app.evaluate(
                `document.querySelector('[data-testid="modal-submit"]').click(); true;`,
            );
            await Bun.sleep(400);
        });
        ...
        test('right-clicking a tab offers rename, save, duplicate and the four closes', async () => {
            ...
            expect(await app.evaluate<boolean>(`${contextItem('Close others')}.disabled`)).toBe(
                true,
            );
            expect(
                await app.evaluate<boolean>(`${contextItem('Close Tabs to the Right')}.disabled`),
            ).toBe(true);
        ...
        ${tab(label)}.querySelector('[data-testid="tab-label"]')
          .dispatchEvent(new MouseEvent('dblclick', { bubbles: true })); true;`;
            ...
            const pressKey = (key: string) =>
                `${renameInput}.dispatchEvent(new KeyboardEvent('keydown', { key: ${JSON.stringify(key)}, bubbles: true })); true;`;
            const setDraft = (text: string) =>
                `${REACT_SETTERS} setNative(${renameInput}, ${JSON.stringify(text)}); true;`;
            ...
            const typeChar = (ch: string) =>
                `document.execCommand('insertText', false, ${JSON.stringify(ch)}); true;`;

            await app.evaluate(dblClickLabel('Query 3'));
            await Bun.sleep(200);
            expect(
                await app.evaluate<number>(
                    `document.querySelectorAll('[data-testid="tab-rename-input"]').length`,
                ),
            ).toBe(1);
        ...
        test('the tab menu opens the inline rename editor', async () => {
            ...
            await app.evaluate(
                `${renameInput}.dispatchEvent(new KeyboardEvent('keydown', { key: 'Escape', bubbles: true })); true;`,
            );
            await Bun.sleep(300);
            expect(await app.evaluate<string[]>(tabLabels)).toContain('Query 3');
        });
        ...
        test('the tab shortcuts open, step through and dock tabs', async () => {
            ...
            await Bun.sleep(400);
            expect(await app.evaluate<string[]>(tabLabels)).toEqual(
                survivors.filter((label) => label !== front),
            );
    ...
    describe('relationship diagram', () => {
      ...
      return true;
        ...
        test('a node dragged past the edge stays on the canvas, in both directions', async () => {
            ...
            await app.evaluate(`${canvas}.scrollLeft = 0; ${canvas}.scrollTop = 0; true;`);
            await Bun.sleep(200);
            const paneBox = await app.evaluate<{
                left: number;
                top: number;
                right: number;
                bottom: number;
        ...
        pickOption(document.querySelector('[data-testid="diagram-db"]'), 'postgres');`);
            await app.waitFor(
                `document.querySelector('[data-testid="diagram-node"]') ? null : true`,
            );
            await Bun.sleep(1500);
            expect(
                await app.evaluate<string>(
                    `document.querySelector('[data-testid="diagram-db"]').getAttribute('data-value')`,
                ),
            ).toBe('postgres');
            expect(
                await app.evaluate<string>(
                    `document.querySelector('[data-testid="diagram"]').textContent`,
                ),
            ).toContain('holds no tables');
            expect(await app.evaluate<string>(treeDatabase)).toBe('shop');
        ...
        test('the diagram re-reads the schema, from its button and from Ctrl+R', async () => {
            ...
            await app.evaluate(
                `document.querySelector('[data-testid="diagram-refresh"]').click(); true;`,
            );
        ...
        test('the menu is not offered while the connect screen is up', async () => {
            ...
            await Bun.sleep(600);
            expect(
                await app.evaluate<boolean>(`!!document.querySelector('[data-menu="Database"]')`),
            ).toBe(false);
        });
    });
    ...
    describe('mysql', () => {
        ...
        test('connects and shows the MySQL badge', async () => {
            expect(
                await app.evaluate<boolean>(`!!document.querySelector('[data-testid="sidebar"]')`),
            ).toBe(true);
            expect(
                await app.evaluate<string>(
                    `document.querySelector('[data-testid="engine-badge"]').textContent`,
                ),
            ).toBe('MySQL');
        });
    ...
    describe('titlebar', () => {
        ...
        [...document.querySelectorAll('[data-testid="window-resize-top"]')].map((el) => ({
          ...
          cursor: getComputedStyle(el).cursor,
            ...
            expect(strips.map((strip) => strip.cursor).sort()).toEqual([
                'nesw-resize',
                'ns-resize',
                'nwse-resize',
            ]);
        ...
        test('the titlebar button opens an assistant tab, and it asks for a key', async () => {
            ...
            await app.evaluate(
                `document.querySelector('[data-testid="titlebar-assistant"]').click(); true;`,
            );
            await app.waitFor(
                `document.querySelector('[data-testid="assistant-panel"]') ? true : null`,
            );
            ...
            expect(
                await app.evaluate<boolean>(
                    `!!document.querySelector('[data-testid="ai-provider-select"]')`,
                ),
            ).toBe(true);
            expect(
                await app.evaluate<boolean>(`!!document.querySelector('[data-testid="ai-key"]')`),
            ).toBe(true);
            ...
            await app.waitFor(
                `[...document.querySelectorAll('[data-testid="tab-label"]')].filter(e => e.textContent === 'Assistant').length === 2 ? true : null`,
            );
            expect(await app.evaluate<string[]>(tabLabels)).toHaveLength(before + 2);

            // They close like any other tab, because they are ones.
            await app.evaluate(
                `${tab('Assistant')}.querySelector('[data-testid="tab-close"]').click(); true;`,
            );
            await app.evaluate(
                `${tab('Assistant')}.querySelector('[data-testid="tab-close"]').click(); true;`,
            );
            await app.waitFor(
                `document.querySelector('[data-testid="assistant-panel"]') ? null : true`,
            );
        });
        ...
        test('the status bar states the assistant, and its menu offers a way to act', async () => {
            ...
            await app.waitFor(
                `document.querySelector('[data-testid="statusbar-assistant"]') ? true : null`,
            );

            await app.evaluate(
                `document.querySelector('[data-testid="statusbar-assistant"]').click(); true;`,
            );
        ...
        return menu.length ? menu : null;
            ...
            await app.waitFor(
                `document.querySelector('[data-testid="context-menu"]') ? null : true`,
            );
        });
        ...
        test('names the connected server', async () => {
            expect(
                await app.evaluate<string>(
                    `document.querySelector('[data-testid="titlebar-title"]').textContent`,
                ),
        ...
        test('the maximise button maximises the real window, and restores it', async () => {
            ...
            if (max) {
                await app.evaluate(
                    `[...document.querySelectorAll('[data-testid="titlebar-btn"]')][1].click(); true;`,
                );
                await Bun.sleep(800);
            }
        ...
        test('the File menu opens, and closes on Escape', async () => {
            ...
            expect(
                await app.evaluate<number>(
                    `document.querySelectorAll('[data-testid="menu-item"]').length`,
                ),
            ).toBe(0);
        });
        ...
        test('Environments manages the picklist the connect form offers', async () => {
            ...
            await app.evaluate(
                `${REACT_SETTERS} setNative(document.querySelector('[data-testid="modal"] input'), 'staging'); true;`,
            );
            await app.evaluate(
        ...
        .find(e => e.querySelector('[data-testid="env-name"]').textContent === 'staging')`;
            ...
            expect(
                await app.evaluate<number>(
                    `document.querySelectorAll('[data-testid="modal"]').length`,
                ),
            ).toBe(0);
        });
        ...
        test('Export and Import connections open their screens, with passwords off', async () => {
            ...
            expect(
                await app.evaluate<boolean>(
                    `document.querySelector('[data-testid="modal"] input[type="checkbox"]').checked`,
                ),
            ).toBe(false);
        ...
        test('the About menu opens, and Version shows the running version', async () => {
            ...
            expect(
                await app.evaluate<string>(
                    `document.querySelector('[data-testid="about-version"]').textContent`,
                ),
        ...
        test('Preferences rebinds Run, and the new key is what runs the query', async () => {
            const chordOf = (id: string) =>
                `document.querySelector('[data-shortcut="${id}"]').textContent`;
            ...
            await app.waitFor(
                `document.querySelector('[data-shortcut="run"]').textContent === 'F8' ? true : null`,
            );
            ...
            await app.evaluate(
                `document.dispatchEvent(new KeyboardEvent('keydown', { key: 'F8', bubbles: true })); true;`,
            );
            await app.waitFor(
            ...
            await app.waitFor(
                `document.querySelector('[data-shortcut="run"]').textContent === 'Ctrl+Enter' ? true : null`,
            );
            await app.evaluate(modalButton('Close'));
            await Bun.sleep(200);
    ...
    describe('saved connections', () => {
        ...
        test('the connect form marks the missing name rather than refusing the click', async () => {
            ...
            await app.evaluate(
                `document.querySelector('[data-testid="saved-new"]')?.click(); true;`,
            );
            await Bun.sleep(300);
            await app.evaluate(`${REACT_SETTERS}
        ...
        true;`);
            ...
            await app.evaluate(
                `${REACT_SETTERS} setNative(document.querySelector('#name'), 'needs-a-name'); true;`,
            );
            await Bun.sleep(200);
            expect(
                await app.evaluate<string | null>(
        ...
        test('the row shows the server it will reach', async () => {
            const label = await app.evaluate<string>(
                `${savedRow('pg-fixture')}.querySelector('[data-testid="saved-server"]').textContent`,
            );
        ...
        test('picking it connects with no password typed', async () => {
            await app.evaluate(
                `${savedRow('pg-fixture')}.querySelector('[data-testid="saved-pick"]').click(); true;`,
            );
            await app.waitFor(
                `document.querySelector('[data-testid="sidebar"]') || document.querySelector('[data-testid="callout"]') ? true : null`,
            );
        ...
        test('a connection that is open says so, and its Edit is refused', async () => {
            ...
            expect(
                await app.evaluate<string>(
                    `${savedRow('pg-fixture')}.querySelector('[data-testid="saved-open"]').textContent`,
                ),
            ).toBe('Open');
            expect(
                await app.evaluate<boolean>(
                    `${savedRow('pg-fixture')}.querySelector('[data-testid="saved-edit"]').disabled`,
                ),
            ).toBe(true);

            // Back to the shell, so the disconnect the next test opens with lands.
            await app.evaluate(
                `document.querySelector('[data-testid="connect-back"]').click(); true;`,
            );
            await Bun.sleep(400);
        });
        ...
        test('editing renames it in place', async () => {
            ...
            expect(
                await app.evaluate<boolean>(
                    `!!${savedRow('pg-fixture')}.querySelector('[data-testid="saved-open"]')`,
                ),
            ).toBe(false);

            await app.evaluate(
                `${savedRow('pg-fixture')}.querySelector('[data-testid="saved-actions"] button').click(); true;`,
            );
            await Bun.sleep(500);
            await app.evaluate(
                `${REACT_SETTERS} setNative(document.querySelector('#name'), 'pg-renamed'); true;`,
            );
            await Bun.sleep(200);
            await app.evaluate(
                `document.querySelector('[data-testid="connect-submit"]').click(); true;`,
        ...
        test('the kept password still connects after an edit that never saw it', async () => {
            await app.evaluate(
                `${savedRow('pg-renamed')}.querySelector('[data-testid="saved-pick"]').click(); true;`,
            );
            await app.waitFor(
                `document.querySelector('[data-testid="sidebar"]') || document.querySelector('[data-testid="callout"]') ? true : null`,
            );
    ...
    describe('testing a connection while typing it', () => {
        ...
        test('a draft is tested where it stands: no name, no record, no connection', async () => {
            ...
            await app.evaluate(
                `${REACT_SETTERS} pickOption(document.querySelector('#type'), 'postgres');`,
            );
            await Bun.sleep(200);
            await app.evaluate(`${REACT_SETTERS}
        ...
        true;`);
            ...
            expect(
                await app.evaluate<boolean>(
                    `document.querySelector('[data-testid="connect-test"]').disabled`,
                ),
            ).toBe(false);

            await app.evaluate(
                `document.querySelector('[data-testid="connect-test"]').click(); true;`,
            );
            expect(
                await app.waitFor<string>(
                    `document.querySelector('[data-testid="connect-test-result"]')?.textContent ?? null`,
                ),
            ...
            await app.evaluate(
                `${REACT_SETTERS} setNative(document.querySelector('#password'), 'not-the-password'); true;`,
            );
            await Bun.sleep(300);
            expect(
                await app.evaluate<boolean>(
                    `!!document.querySelector('[data-testid="connect-test-result"]')`,
                ),
            ).toBe(false);
            ...
            expect(
                await app.waitFor<string>(
                    `document.querySelector('[data-testid="connect-test-error"]')?.textContent ?? null`,
                ),
        ...
        test('an edit tests with the password it was never shown', async () => {
            ...
            await app.evaluate(
                `${savedRow('pg-test-draft')}.querySelector('[data-testid="saved-edit"]').click(); true;`,
            );
            await app.waitFor(
                `document.querySelector('[data-testid="connect-test"]') ? true : null`,
            );
    ...
    describe('session restore', () => {
        ...
        test('reconnecting reopens the tabs, the query text and the tables', async () => {
            ...
            await app.evaluate(
                `${savedRow('pg-session')}.querySelector('[data-testid="saved-pick"]').click(); true;`,
            );
            await app.waitFor(
                `JSON.stringify(${tabLabels}) === JSON.stringify(['Query 1', 'users']) ? true : null`,
            );
    ...
    describe('saved queries', () => {
      ...
      [...document.querySelectorAll('[data-testid="saved-query-pick"]')]
        ...
        async function deleteFirstSavedQuery(): Promise<void> {
            ...
            await app.waitFor(
                `document.querySelector('[data-testid="saved-query-delete"]') ? true : null`,
            );
            await app.evaluate(
                `document.querySelector('[data-testid="saved-query-delete"]').click(); true;`,
            );
            await Bun.sleep(200);
        ...
        test('Ctrl+S names it, and the tab takes that name', async () => {
            ...
            await app.waitFor(
                `document.querySelector('[data-testid="save-query-name"]') ? true : null`,
            );
            expect(
                await app.evaluate<string>(
                    `document.querySelector('[data-testid="save-query-name"]').value`,
                ),
            ).toBe('Query 1');

            await app.evaluate(
                `${REACT_SETTERS} setNative(document.querySelector('[data-testid="save-query-name"]'), 'ui-saved'); true;`,
            );
            await Bun.sleep(150);
            await app.evaluate(
        ...
        test('the picker opens it into a new tab, seeded and runnable', async () => {
            await app.evaluate(openPicker);
            await app.waitFor(
                `document.querySelector('[data-testid="saved-queries-panel"]') ? true : null`,
            );
        ...
        test('saving one copy lands in every other tab open on the same query', async () => {
            ...
            await app.evaluate(
                `document.querySelectorAll('[data-testid="tab-pick"]')[0].click(); true;`,
            );
            await Bun.sleep(500);
        ...
        test('deleting it empties the list and leaves the open tab holding the text', async () => {
            ...
            await app.evaluate(
                `[...document.querySelectorAll('button')].find(b => b.textContent === 'Cancel').click(); true;`,
            );
            await Bun.sleep(300);
            expect(await app.evaluate<boolean>(dialogOpen)).toBe(false);
        });
    ...
    describe('workspaces', () => {
        ...
        test('one workspace is skipped, and the bar names the one you are in', async () => {
            ...
            expect(
                await app.evaluate<string>(
                    `document.querySelector('[data-testid="ws-bar-name"]').textContent`,
                ),
            ).toBe('Default');
        ...
        test('the bar is the way to the picker, which is how a second one is made', async () => {
            ...
            await Bun.sleep(400);
            await app.evaluate(
                `${REACT_SETTERS} setNative(document.querySelector('#workspace-name'), 'Acme'); true;`,
            );
            await Bun.sleep(200);
            await app.evaluate(
                `document.querySelector('[data-testid="connect-submit"]').click(); true;`,
        ...
        test('a second workspace makes the picker the way in, counting what each holds', async () => {
            ...
            expect(
                await app.evaluate<string[]>(
                    `[...document.querySelectorAll('[data-testid="ws-count"]')].map(e => e.textContent)`,
                ),
        ...
        test('deleting one says what it will cost, then takes its connections with it', async () => {
            ...
            expect(
                await app.evaluate<string>(
                    `${savedRow('Default')}.querySelector('[data-testid="saved-delete"]').title`,
                ),
    ...
    describe('multiple connections', () => {
        ...
        test("two connections holding a database called 'shop' do not share its tables", async () => {
            ...
            await Bun.sleep(400);
            await app.evaluate(selectDatabase('shop'));
            await app.waitFor(`(${schemaLabels}).includes('reporting') ? true : null`);
        ...
        test("each connection's tab runs against that connection", async () => {
            ...
            const gridFirstCell = `document.querySelector('.grid tbody td:not(.gutter)')?.textContent ?? ''`;
            await app.waitFor(`(${gridFirstCell}).includes('8.') ? true : null`);
            const mysql = await app.evaluate<string>(gridFirstCell);
            ...
            const gridFirstCellPg = `document.querySelector('.grid tbody td:not(.gutter)')?.textContent ?? ''`;
            await app.waitFor(`(${gridFirstCellPg}).includes('PostgreSQL') ? true : null`);
            const pg = await app.evaluate<string>(gridFirstCellPg);
        ...
        test('disconnecting the last one is the way back to the connect screen', async () => {
            ...
            await app.waitFor(`document.querySelector('[data-testid="rail"]') ? null : true`);
            expect(
                await app.evaluate<boolean>(`!!document.querySelector('[data-testid="rail"]')`),
            ).toBe(false);
            expect(
                await app.evaluate<boolean>(

...
export const postgresDriver: Driver<pg.Client> = {
    ...
    async listRelationships(client) {
        // The same pg_attribute read `listColumns` makes, widened from one relation
        // to every ordinary table in the database -- see that method for why
        // format_type and the pg_index join are what answer the type and the key.
        //
        // `relkind IN ('r', 'p')` is tables and partitioned tables and nothing else:
        // a view has no constraint of its own and cannot be referenced, so it could
        // only be a node no line reaches. `relispartition` drops the individual
        // partitions, exactly as `listTables` does -- they carry their parent's
        // columns and would draw the same table N times.
        ...
        // `listColumns`' constraint read with its relation filter removed, so the
        // whole database's foreign keys arrive in one pass. The conkey/confkey
        // unnest is what pairs a local column with the one it actually points at;
        // ordering by `ck.ord` is what keeps a composite key's columns in key order,
        // which `assembleDiagram` relies on and does not re-derive.
    ...
    async tableDdl(client, ref, kind) {
        ...
        const columnLines = (cols.rows as unknown[][]).map((r) => {
            ...
            else if (generated === 's' && defExpr)
                line += ` GENERATED ALWAYS AS (${defExpr}) STORED`;

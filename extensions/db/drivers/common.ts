 ...
 */
const relationKey = (schema: string | undefined, table: string): string => JSON.stringify([schema ?? '', table]);
 ...
 */
export function assembleDiagram(columns: DiagramColumnPart[], links: DiagramLinkPart[]): DiagramTable[] {
  ...
  for (const part of columns) {
    ...
    table.columns.push({ name: part.name, dataType: part.dataType, primaryKey: part.primaryKey });
  }

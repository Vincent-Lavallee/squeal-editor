...
const COMMANDS: Handlers = {
    ...
    async 'db.columns'({ connectionId, database, table, schema }) {
        return {
            columns: await getConnection(connectionId).listColumns(database, { table, schema }),
        };
    },
    ...
    async 'db.triggers'({ connectionId, database, table, schema }) {
        return {
            triggers: await getConnection(connectionId).listTriggers(database, { table, schema }),
        };
    },

    async 'db.triggerDdl'({ connectionId, database, table, schema, trigger }) {
        return {
            ddl: await getConnection(connectionId).triggerDdl(database, { table, schema }, trigger),
        };
    },
    ...
    async 'ai.send'({ turnId, model, messages, tools }) {
        return {
            message: await sendTurn(turnId, model, messages, tools, (text) =>
                send(AI_DELTA_EVENT, { turnId, text }),
            ),
        };
    },

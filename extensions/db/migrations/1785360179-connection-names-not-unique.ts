 ...
 */
/**
 * Lifts `stars` and `connection_sessions` into carry tables and drops the
 * originals, so `saved_connections` can be dropped and rebuilt without taking
 * either child down with it under `PRAGMA foreign_keys = ON`.
 */
function liftChildren(db: Database): void {
...
/**
 * Rebuilds `saved_connections` without `UNIQUE (workspace_id, name)`. `color`
 * is `COALESCE`d rather than copied straight across -- see the file comment.
 */
function rebuildSavedConnections(db: Database): void {
    db.run(`
...
/** Recreates `stars` and `connection_sessions` against the rebuilt table and restores their rows. */
function restoreChildren(db: Database): void {
    db.run(`
      CREATE TABLE stars (
        id            TEXT PRIMARY KEY,

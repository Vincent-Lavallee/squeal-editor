...
import type { AwsCredentialStatus, AwsSsoPrompt, ConnectionConfig } from '../../shared/protocol/index.ts';
 ...
 */
export async function ssoLogin(profile: string, onPrompt: (prompt: AwsSsoPrompt) => void): Promise<void> {
  ...
  try {
    ...
    throw new Error('The AWS CLI was not found. Install it, or run `aws sso login` yourself and try again.');
  }
 ...
 */
export async function readPrompts(stdout: ReadableStream<Uint8Array>, onPrompt: (prompt: AwsSsoPrompt) => void): Promise<string> {
...
export async function credentialStatus(profile: string): Promise<AwsCredentialStatus> {
  const trimmed = profile.trim();
  if (!trimmed) return { valid: false, problem: 'Name an AWS profile first.', signInHelps: false };
...
export function mapAwsError(err: unknown, profile: string): Error {
  ...
  switch (awsFailureKind(err)) {
    ...
    case 'missing-profile':
      return new Error(`AWS profile "${profile}" was not found. Check the profile name and your AWS configuration.`);
    default:

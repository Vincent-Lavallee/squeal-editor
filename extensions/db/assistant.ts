...
import { providerLabel, type AiMessage, type AiModel, type AiProvider, type AiStatus, type AiToolDef } from '../../shared/protocol/index.ts';
...
const ENDPOINTS: Record<AiProvider, Endpoint> = {
  ...
  openai: {
    ...
    keeps: /^(gpt-|o[1-9])/,
    rejects: /(audio|realtime|transcribe|tts|image|moderation|embedding|instruct|search|dall-e|whisper)/,
...
export async function connect(provider: AiProvider, key: string): Promise<AiStatus> {
  ...
  await Bun.secrets.set({ service: KEYCHAIN_SERVICE, name: CREDENTIAL_NAME, value: JSON.stringify({ provider, key: trimmed }) });
...
function stated(detail: string): string | undefined {
  try {
    const body = JSON.parse(detail) as { error?: { message?: string } | string; message?: string };
...
function providerFailure(provider: AiProvider, statusCode: number, detail: string): Error {
  ...
  if (statusCode === 401 || statusCode === 403) return new Error(said ?? `${label} rejected this API key.`);
  if (statusCode === 402) return new Error(said ?? `${label} says this account has no credit.`);
  if (statusCode === 429) return new Error(said ?? `${label} is rate limiting this key. Try again shortly.`);
...
async function fetchModels(provider: AiProvider, key: string): Promise<AiModel[]> {
  ...
  const all = entries
    ...
    .map((entry) => ({
      ...
      publishedAt: entry.created_at ? Date.parse(entry.created_at) : (entry.created ?? 0) * 1000,
    }))
    .sort((left, right) => right.publishedAt - left.publishedAt);

  const usable = all.filter((model) => endpoint.keeps.test(model.id) && !endpoint.rejects?.test(model.id));
  const offered = usable.length ? usable : all;
  if (!offered.length) throw new Error(`${providerLabel(provider)} listed no models for this key.`);

  const preferred = endpoint.prefers.map((pattern) => offered.find((model) => pattern.test(model.id))).find(Boolean) ?? offered[0];
...
export async function send(
  ...
  try {
    const response = await fetch(`${endpoint.base}/${anthropic ? 'messages' : 'chat/completions'}`, {
      method: 'POST',
      ...
      body: JSON.stringify(anthropic ? anthropicBody(model, messages, tools) : openAiBody(model, messages, tools)),
    });

    if (!response.ok) await refuse(provider, response);
    if (!response.body) throw new Error(`${providerLabel(provider)} returned an empty response.`);

    return anthropic ? await readAnthropicStream(response.body, onDelta) : await readOpenAiStream(response.body, onDelta);
  } finally {
    inFlight.delete(turnId);
  }
}
 ...
 */
async function eachPayload(body: ReadableStream<Uint8Array>, onPayload: (payload: string) => void): Promise<void> {
...
function openAiBody(model: string, messages: AiMessage[], tools: AiToolDef[]): Record<string, unknown> {
  return {
    model,
    stream: true,
 ...
 */
function anthropicBody(model: string, messages: AiMessage[], tools: AiToolDef[]): Record<string, unknown> {
  ...
  for (const message of messages) {
    ...
    for (const call of message.toolCalls ?? []) {
      ...
      blocks.push({ type: 'tool_use', id: call.id, name: call.name, input: parsePayload<Record<string, unknown>>(call.arguments || '{}') ?? {} });
    }

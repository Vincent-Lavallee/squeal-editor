...
function msvc(compiler: string) {
  return [
    compiler, "/nologo", "/O1", "/MT", "/LD", "/W3",
    SOURCE,
    `/Fo:${join(BUILD_DIR, "squeal-window-chrome.obj")}`,
    `/Fe:${LINKED}`,
    "/link", "user32.lib", `/IMPLIB:${join(BUILD_DIR, "squeal-window-chrome.lib")}`,
  ];
}
...
function developerPrompt(): string[] | null {
  ...
  const found = Bun.spawnSync([vswhere, "-latest", "-products", "*",
  ...
  const compile = msvc("cl").map((arg) => (arg.includes(" ") ? `"${arg}"` : arg)).join(" ");
...
if (!command) {
  ...
  console.warn(`${message} The app will run without the window chrome: the titlebar keeps its 7px band and minimise/maximise do not animate.`);
  process.exit(0);
}
